    ...
    public boolean onEnd() {
     ...
     private String getCondition() {
         if (mParams != null) {
             ...
             List<SubInfoRecord> simInfoList = SubscriptionManager.getActiveSubInfoList();
             if(simInfoList != null && simInfoList.size() > 0) {
                 for(SubInfoRecord simInfo : simInfoList){
                     if(mParams.contains(String.valueOf(simInfo.slotId))) {
                         conditionList.add(Long.toString(simInfo.subId));
                     }
                 }
             }
