...
*/
    ...
    public void configSmsPreferenceEditorWhenRestore(Activity hostActivity, SharedPreferences.Editor editor) {
        ...
        if (MTK_GEMINI_SUPPORT) {
            ...
            */
        	List<SubInfoRecord> simList = SubscriptionManager.getActiveSubInfoList();
            if (simList != null) {
                int simCount = simList.size();
                for (int index = 0; index < simCount; index ++) {
                    long subId = simList.get(index).subId;
        ...
        if (MTK_GEMINI_SUPPORT) {
            ...
            */
            ...
            if (simList != null) {
                ...
                for (int index = 0; index < simCount; index ++) {
                    Log.d(TAG,"set simId_MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY false");
                    Long simId = simList.get(index).subId;
                    editor.putBoolean(Long.toString(simId) + "_" + MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY, false);
                }
            }
        } else {
    ...
    public Bundle getSmsValidityParamBundleWhenSend(Context context, long subId) {
...
//            extra.putInt(SmsManager.EXTRA_PARAMS_VALIDITY_PERIOD, valid);
    ...
    private void addSmsValidityPeriodPreference(final Activity hostActivity, PreferenceCategory pC, int simCount) {
        ...
        if (MTK_GEMINI_SUPPORT) {
            ...
            */
            final List<SubInfoRecord> simList = SubscriptionManager.getActiveSubInfoList();
                ...
                if (simCount == 0) {
                    ...
                    p.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
                        ...
                        public boolean onPreferenceClick(Preference preference) {
                            ...
                            */
                            long subId = simList.get(0).subId;
    ...
    public boolean handleSelectCardPreferenceTreeClick(Activity hostActivity, final long subId) {
        /* if (!EncapsulationConstant.USE_MTK_PLATFORM) {
            // if not mtk platform, no this feature
           return;
        } */
    ...
    private String getSmsValidityKeyBySubId(long subId) {
        return Long.toString(subId) + "_" + SMS_VALIDITY_PERIOD_PREFERENCE_KEY;
    }
    private void showSmsValidityPeriodDialog(Context context, long subId) {
    ...
    private int getSmsValiditybySubId(Context context, long subId) {
/*
...
//            }
            ...
            */
            ...
            if (simCount > 1) {
                ...
                /* temp close for build , as com.mediatek.telephone can not ready
                    ...
                    */
                    Long simId = simList.get(0).subId;
                    cp.setKey(Long.toString(simId) + "_" + MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY);
    ...
    public boolean configMmsPreferenceState(Activity hostActivity, String preference, long subId, CheckBoxPreference cp) {
        ...
        if (preference != null && preference.equals(MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY)) {
            boolean checked = prefs.getBoolean(Long.toString(subId) + "_" + MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY, false);
    ...
    public boolean setMmsPreferenceState(Activity hostActivity, String preference, long subId, boolean checked) {
        ...
        if (preference.equals(MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY)) {
            editor.putBoolean(Long.toString(subId) + "_" + MMS_ENABLE_TO_SEND_DELIVERY_REPORT_KEY, checked);
            return true;
        }
        return false;
    }
    ...
    public void setAcknowledgeDeliveryReport(Context mContext, long subId, AcknowledgeInd acknowledgeInd) {
        // X-Mms-Report-Allowed Optional
        boolean reportAllowed = isEnableSendDeliveryReport(mContext, (int) subId);
    ...
    public void setNotifyRespDeliveryReport(Context mContext, long subId, NotifyRespInd notifyRespInd) {
       // X-Mms-Report-Allowed Optional
       boolean reportAllowed = isEnableSendDeliveryReport(mContext, (int) subId);
       Xlog.d(TAG, "reportAllowed: " + reportAllowed);
