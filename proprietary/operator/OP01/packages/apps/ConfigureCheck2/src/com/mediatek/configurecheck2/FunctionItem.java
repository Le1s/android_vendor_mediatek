...
class CheckLabAPN extends CheckItemBase {
    private List<SubInfoRecord> mSimInfoList = null;
    ...
    public check_result getCheckResult() {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
            ...
            for (SubInfoRecord simInfo : mSimInfoList) {  
                int simNo = simInfo.slotId + 1;              
                long subid = getSubIdBySlot(simInfo.slotId);
                ...
                if (cursor_APN == null || !cursor_APN.moveToNext()) {
                    ...
                    mSimNeedSetAPN.add(new Integer(simInfo.slotId));
                } else {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
            for (Integer simId : mSimNeedSetAPN) {
                ...
                if ((cursor != null) && (true == cursor.moveToFirst())) {
                    ...
                    long subid = getSubIdBySlot(simId.intValue());
                    getContext().getContentResolver().update(
                                    Uri.parse("content://telephony/carriers/preferapn" + "/subId/" + subid),
                                    values_prefer, 
                                    null, 
...
class APNBuilder {
    ...
    public void setAsCurrent() {
        ...
        if (mSlot == -1) {
            ...
            //int simNo = mSlot + 1;
            long subid = getSubIdBySlot(mSlot);
            mContext.getContentResolver().update(
                                Uri.parse("content://telephony/carriers/preferapn" + "/subId/" + subid),
                                values, 
                                null, 
            ...
            switch (msg.what) {
                case MSG_GET_SMSC_DONE:
                    ...
                    if(SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                        ...
                        for (SubInfoRecord simInfo : mSimInfoList) {
                            String curStr = bd.getString("SMSC" + simInfo.slotId);

                            CTSCLog.v("CheckSMSC", "SMSC" + simInfo.slotId + " " + curStr);
                            if (false == curStr.equalsIgnoreCase("+8613800100569")) {
                                if(smscStr.toString().length() != 0) {
                                    smscStr.append("\n");
                                }
                                smscStr.append("Sim" + (simInfo.slotId+1) + getContext().getString(R.string.smsc_not_correct));
    ...
    public boolean onReset() {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                if(SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
                    for (SubInfoRecord simInfo : mSimInfoList) {
                        telephonyMgr.setScAddress(simInfo.slotId, "+8613800100569");
                    }
                } else {
    ...
    public boolean onReset() {
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1") == true) {
            for (Integer simSlotId : mSimNeedSetAPN) {                
                long subId = getSubIdBySlot(simSlotId);
