...
public class ServiceStateExtOP01 extends DefaultServiceStateExt {
    ...
    public String onUpdateSpnDisplay(String plmn, ServiceState ss, long subId) {
        //[ALPS01663902]-Start
        if (plmn == null) {
            return plmn;
        }
        ...
        if (isRoming) {
            String prop1 = TelephonyManager.getTelephonyProperty(
                    TelephonyProperties.PROPERTY_ICC_OPERATOR_ALPHA, subId, "");
            log("getSimOperatorName simId = " + subId + " prop1 = " + prop1);
            if (prop1.equals("")) {
                String prop2 = TelephonyManager.getTelephonyProperty(
                        TelephonyProperties.PROPERTY_ICC_OPERATOR_DEFAULT_NAME, subId, "");
                log("getMTKdefinedSimOperatorName simId = " + subId + " prop2 = " + prop2);
                if (!prop2.equals("")) {
                    plmn = plmn + "(" + prop2 + ")";
                }
            } else {
        ...
        if ((phoneId == (switchStatus - 1)) && SubscriptionManager.isValidPhoneId(phoneId)){
            ...
            if (isSimType) {//SIM
                ...
                if (userNetworkMode >= Phone.NT_MODE_WCDMA_PREF){
                    log("needSwitchRatMode: userNetworkMode=" + userNetworkMode);
    ...
    private boolean isWorldPhoneSupport() {
        if(SystemProperties.getInt(TelephonyProperties.PROPERTY_WORLD_PHONE, 0) == 1) {
            return true;
        }
        return false;
    }
