...
public class SimUtils {
    ...
    private static long sCurSubId = -1;
    ...
    public static void setCurrentSubId(long subId) {
        sCurSubId = subId;
    ...
    public static long getCurSubId() {
        LogUtils.d(TAG, "getCurSlotId() sCurSubId=" + sCurSubId);
        return sCurSubId;
    }

    public static String getAccountTypeBySub(long subId) {
    ...
    private static int getSimTypeBySub(long subId) {
        final ITelephonyEx iTel = ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE_EX));
        int simType = SIM_TYPE_SIM;
        try {
            if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
     ...
     * @return
     */
    public static boolean refreshAASList(long subId) {
     ...
     * @return
     */
    public static List<AlphaTag> getAAS(long subId) {
     ...
     * @return
     */
    public static int getAASTextMaxLength(long subId) {
        int slot = SubscriptionManager.getSlotId(subId);
        if (slot < SLOT_ID1 || slot > SLOT_ID2) {
    ...
    public static String getAASById(long subId, int index) {
        int slotId = SubscriptionManager.getSlotId(subId);
    ...
    public static int getAasIndexByName(String aas, long subId) {
          int slotId = SubscriptionManager.getSlotId(subId);
    ...
    public static int insertUSIMAAS(long subId, String aasName) {
         int slotId = SubscriptionManager.getSlotId(subId);
    ...
    public static boolean updateUSIMAAS(long subId, int index, int pbrIndex, String aasName) {
          int slotId = SubscriptionManager.getSlotId(subId);
        if (slotId < SLOT_ID1 || slotId > SLOT_ID2) {
            return false;
        }
    ...
    public static boolean removeUSIMAASById(long subId, int index, int pbrIndex) {
     int slotId = SubscriptionManager.getSlotId(subId);
        if (slotId < SLOT_ID1 || slotId > SLOT_ID2) {
            return false;
        }
    ...
    public static boolean isAasTextValid(String text, long subId) {
     ...
     * @return
     */
    public static int getAASMaxCount(long subId) {
        int slot = SubscriptionManager.getSlotId(subId);
        if (slot < SLOT_ID1 || slot > SLOT_ID2) {
            return ERROR;
        }
    ...
    public static int getAnrCount(long subId) {
        int slot = SubscriptionManager.getSlotId(subId);
        if (slot < SLOT_ID1 || slot > SLOT_ID2) {
            return ERROR;
        }
