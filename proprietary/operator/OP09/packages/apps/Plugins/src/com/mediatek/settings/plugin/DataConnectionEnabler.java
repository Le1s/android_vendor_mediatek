...
public final class DataConnectionEnabler implements CompoundButton.OnCheckedChangeListener {
    ...
    static final int DETACH_DATA_CONN_TIME_OUT = 10000;
    static final int ATTACH_DATA_CONN_TIME_OUT = 30000;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (DATA_SWITCH_TIME_OUT_MSG == msg.what) {
                Log.i(TAG, "reveive time out msg...");
                mIsDataConnectActing = false;
                updateSwitcherState();
            }
        }
     ...
     * @param context DataConnectionSettings'context
     * @param switchs swith button for data on/off
     */
    ...
    public void resume() {
        ...
        updateSwitcherState();
        Log.d(TAG, "resume mConnectivityManager.getMobileDataEnabled()="
    ...
    /**
     * set switch button.
     * @param switchs which switchbutton will used
     */
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        if (isChecked != mConnectivityManager.getMobileDataEnabled()) {
            ...
            mHandler.sendEmptyMessageDelayed(DATA_SWITCH_TIME_OUT_MSG,
                    isChecked ? ATTACH_DATA_CONN_TIME_OUT
    ...
    static boolean isGPRSEnable(Context context) {
        ...
        Log.d(TAG, "isMMSProcess=" + isMMSProcess +
                " isRadioOff=" + isRadioOff + ", hasSimUnLocked=" + hasSimUnLocked);
        return !isMMSProcess && !isRadioOff && hasSimUnLocked;
    }

    private static boolean hasSlotRadioOnNotLocked(Context context) {
        for (int slot = PhoneConstants.GEMINI_SIM_1;
            slot < PhoneConstants.GEMINI_SIM_NUM; slot ++) {
    ...
    static int getSimIndicator(Context context,  int slotId) {
        Log.d(TAG, "getSimIndicator---slotId=" + slotId);
        if (!isTargetSlotRadioOn(slotId)) {
            Log.d(TAG, "Force the state to be radio off as airplane mode or dual sim mode");
    ...
    static boolean isTargetSlotRadioOn(int slotId) {
        try {
            ...
            Log.d(TAG, "isTargetSlotRadioOn query error, return false");
        }
        return false;
    }
    ...
    public static int getSimIndicator(ContentResolver resolver, int slotId) {
        ...
        boolean isAirplaneOn = Settings.System.getInt(resolver,
                Settings.System.AIRPLANE_MODE_ON, -1) == 1;
        if (isAirplaneOn) {
            Log.d(TAG, "isAirplaneOn = " + isAirplaneOn);
            return PhoneConstants.SIM_INDICATOR_RADIOOFF;
        }
