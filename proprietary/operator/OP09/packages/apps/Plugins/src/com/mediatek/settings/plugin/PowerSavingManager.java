...
public class PowerSavingManager {
    ...
    private static final String ENABLE_CPU_POWER_SAVING_COMMAND =
        "/system/bin/thermal_manager /etc/.tp/thermal.conf";
    private static final String DISABLE_CPU_POWER_SAVING_COMMAND =
        "/system/bin/thermal_manager /etc/.tp/thermal.off.conf";
     ...
     * @return true if saving mode is on
     */
    ...
    public void turnOnPowerSavingMode(String key) {
        ...
        if (KEY_CPU.endsWith(key)) {
            Log.d(TAG, "Enable CPU saving is checked");
            enableCpuSaving(true);
            ...
            Log.w(TAG, "Wrong preference key: " + key);
        }
    }
    ...
    private void turnOffPowerSavingMode() {
        if (!mSharePref.getBoolean("CPU", true)) {
            Log.d(TAG, "Turn Off Power Saving Mode!");
            Log.d(TAG, "Disable CPU saving...");
    ...
    public boolean turnOffWifi() {
        ...
        if (wifiManager.isWifiEnabled()) {
            Log.d(TAG, "Wifi is enable,try trun off ...");
    ...
    public boolean turnOffBlueTooth() {
        ...
        if (blueToothAdapter != null && blueToothAdapter.getState() == BluetoothAdapter.STATE_ON) {
            Log.d(TAG, "Bluetooth is enable,try trun off ...");
     ...
     * @param isStart enable or disable
     */
    public void enableCpuSaving(boolean isStart) {
        String command = isStart ?
                ENABLE_CPU_POWER_SAVING_COMMAND : DISABLE_CPU_POWER_SAVING_COMMAND;
    ...
    public int getCpuState() {
        ...
        try {
            java.lang.Process process =
                Runtime.getRuntime().exec(CAT_CPU_POWER_SAVING_STATUS_COMMAND);
            ...
            try {
                ...
                Log.e(TAG, "exe shell command InterruptedException: " + e.getMessage());
                result = 1;
            }
        } catch (IOException e) {
            Log.e(TAG, "exe shell command IOException: " + e.getMessage());
            result = 1;
        } finally {
            if (null != bufferedReader) {
                try {
                    ...
                    Log.e(TAG, "close reader in finally block exception: " + e.getMessage());
                }
            }
        }
    ...
    public int getTimoutValue() {
        ...
        final CharSequence[] valuesTimeout =
            mContext.getResources().getStringArray(R.array.screen_timeout_values);
    ...
    /**
     * Refer com.mediatek.audioprofile.AudioProfileSettings KEY.
     * updateActivePreference Editprofile.java KEY mProfileManager
     */
    ...
    public void setBrightness(int brightness) {
        try {
            ...
            Log.e(TAG, "set backlight brightness fail", doe);
        }
        Settings.System.putInt(mContext.getContentResolver(),
