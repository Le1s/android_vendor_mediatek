...
/**
 * CT feature customize DateTime Status and add roaming time.
 */
public class OP09DateTimeSettingsExtImp extends DefaultDateTimeSettingsExt {
    ...
    private static final String ACTION_TIME_MODE =
        "com.mediatek.settings.intent.action.CT_TIME_ROAMING_SETTING";
    private static final String PACKAGE_NAME = "com.mediatek.op09.plugin";
    private static final String TIMEZONECLASS_NAME =
     ...
     * It will be called onRume life cycle callback,
     * to ensure the status is always right,use in DateTImeSettings
     * @param context DateTimeSettings
     * @param listPreference AutoTimePref
     * @param checkBoxPreference AutoTimeZonePref
     */
    @Override
    public void customizeDateTimePreferenceStatus(Context context,
        ...
        if (ENABLE_CT_TIANYI_DATE_TIME_FEATURE) {
            ...
            String sim2Nitz = "";
            String defaultValue = "";
            Log.d(TAG, "op09 getDateTimeStatus");
            ...
            Log.d(TAG, "sim slot 1 phoneType = " + phoneType);
            if (simInfoList != null && simInfoList.size() == DOUBLE_SIM_CARD) {
                ...
                Log.i(TAG, "getDateTimeStatus Error: no SIM inserted");
            }
            if (!isEnabled) {
    ...
    private boolean isEnableDateTime(boolean isEnabled, int phoneType,
        ...
        if (phoneType == PhoneConstants.PHONE_TYPE_GSM) {
            sim1Nitz = SystemProperties.get("gsm.nitz.time", defaultValue);
            sim2Nitz = SystemProperties.get("gsm.nitz.time.2", defaultValue);
            Log.d(TAG, "GG dual stand by, sim1Nitz is :" + sim1Nitz + "; sim2Nitz is :" + sim2Nitz);
            if (!("".equals(sim1Nitz)) && !("".equals(sim2Nitz))) {
                isEnabled = false;
                Log.d(TAG, "Both network support NITZ");
            } else {
                Log.d(TAG, "One of not support NITZ so enabled");
            }
        } else if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
            isEnabled = false;
            Log.d(TAG, "SIM1 registered CDMA network");
        } else {
            Log.d(TAG, "None type");
        }
        return isEnabled;
    ...
    private void setPrefEnableState(Context context, PreferenceScreen timeZone,
        ...
        if (!isRoaming) {
            ...
            if (simInformation.insertedSimSlot() == SimInformation.TWO_SIM_INSERTED) {
                Log.d(TAG, "timezone status: two sim insearted");
                if (phoneType == PhoneConstants.PHONE_TYPE_GSM) {
                    sim1Nitz = SystemProperties.get("gsm.nitz.time",
                            defaultValue);
                    sim2Nitz = SystemProperties.get("gsm.nitz.time.2",
                            defaultValue);
                    Log.d(TAG, "GG dual stand by, sim1Nitz is :" +
                            sim1Nitz + "; sim2Nitz is :" + sim2Nitz);
                    if (!("".equals(sim1Nitz)) && !("".equals(sim1Nitz))) {
                        timeZone.setEnabled(false);
                        Log.d(TAG, "sim1 and sim2 setEnabled false");
                    }
                } else if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                    timeZone.setEnabled(false);
                    Log.d(TAG, "SIM1 registered CDMA network.");
                ...
                Log.d(TAG, "timezone status: only Slot1 one sim insearted");
                if (phoneType == PhoneConstants.PHONE_TYPE_GSM) {
                    sim1Nitz = SystemProperties.get("gsm.nitz.time",
                            defaultValue);
                    Log.d(TAG, "Only inserted SIM1, sim1Nitz is :" + sim1Nitz);
                    if (!("".equals(sim1Nitz))) {
                        timeZone.setEnabled(false);
                        Log.d(TAG, "Slot1 registered GSM, and supports NITZ");
                    }
                } else if (phoneType == PhoneConstants.PHONE_TYPE_CDMA) {
                    timeZone.setEnabled(false);
                    Log.d(TAG, "tstatus: Slot1 registered CDMA supports NITZ");
                } else {
                    Log.d(TAG, "phoneType :" + phoneType);
                }
                ...
                Log.d(TAG, "Only Slot2 insearted, sim2Nitz is: " + sim2Nitz);
                if (!("".equals(sim2Nitz))) {
                    timeZone.setEnabled(false);
                    Log.d(TAG, "timezone status: Slot2 support NITZ");
                }
            }
        }
    }
