 ...
 */
public class GeneralCallSettings extends PreferenceFragment
    ...
    private static final String ESURFING_DIAL_ACTIVITY_NAME =
        "com.mediatek.calloption.plugin.ESurfingGuideSelectActivity";
    ...
    private static final int PREFERRED_TTY_MODE = Phone.TTY_MODE_OFF;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                Log.d(LOG_TAG, "mReceiver [action = " + action + "]");
        ...
        if (!isSupportFeature(FEATURE_DUAL_MIC)) {
    ...
    private void updateScreen() {
        ...
        if (mButtonEsurfing != null) {
            try {
                mButtonEsurfing.setChecked(
                        Settings.System.getInt(getActivity().getContentResolver(),
                        Settings.System.ESURFING_DIALING) == 1 ? true : false);
            } catch (SettingNotFoundException e) {
                Log.e(LOG_TAG, "onResume query settings exception");
            }
        }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference.equals(mButtonEsurfingGuide)) {
            ...
            Log.d(LOG_TAG, "onPreferenceTreeClick mButtonMr.isChecked()=" + mButtonMr.isChecked());
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference.equals(mButtonDualMic)) {
            Log.d(LOG_TAG, "onPreferenceChange mButtonDualMic.isChecked()=" +
                    mButtonDualMic.isChecked());
    ...
    private void setDualMicMode(String dualMic) {
        ...
        if (context != null) {
            ...
            Log.e(LOG_TAG, "setDualMicMode, dualMic set error, dualMic=" + dualMic);
        }
    }
    ...
    private void handleTTYChange(Preference preference, Object objValue) {
        ...
        int settingsTtyMode =
            android.provider.Settings.Secure.getInt(getActivity().getContentResolver(),
                android.provider.Settings.Secure.PREFERRED_TTY_MODE, PREFERRED_TTY_MODE);
        Log.d(LOG_TAG, "handleTTYChange: requesting set TTY mode enable (TTY) to" +
    ...
    /**
     * tools for feature option support or not.
     * @param feature featureoption
    ...
    public static boolean isSupportFeature(String feature) {
        ...
        if (feature.equals(FEATURE_TTY)) {
            return Utils.isTtySupport();
        } else if (feature.equals(FEATURE_DUAL_MIC)) {
            return Utils.isDualMic();
        }
        return false;
    }
    ...
    private boolean slotRadioOn(int slotId) {
        ...
        try {
            if (mTelephonyEx != null) {
                ...
                Log.d(LOG_TAG, "Slot " + slotId + " is in radion state " + isRadioOn);
            }
        } catch (RemoteException e) {
            Log.w(LOG_TAG, "mTelephony exception");
        }
        return isRadioOn;
    }
    /// M: getMrState from phone's shared preferences
    private static boolean getMrState() {
        return PhoneFactory.getDefaultPhone().getContext().
            getSharedPreferences(PHONE_PREFERENCE_NAME, Context.MODE_PRIVATE).
    ...
    private static void setMrState(boolean newState) {
        PhoneFactory.getDefaultPhone().getContext().
            getSharedPreferences(PHONE_PREFERENCE_NAME, Context.MODE_PRIVATE).
            edit().putBoolean(BUTTON_OTHERS_MINUTE_REMINDER_KEY, newState).commit();
    }
}
