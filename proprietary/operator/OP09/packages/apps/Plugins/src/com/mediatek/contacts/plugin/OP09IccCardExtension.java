...
import com.mediatek.contacts.ext.DefaultIccCardExtension;
import com.mediatek.op09.plugin.R;
import com.mediatek.telephony.SimInfoManager.SimInfoRecord;

public class OP09IccCardExtension extends DefaultIccCardExtension {
    ...
    public Drawable getIconDrawableBySimInfoRecord(SimInfoRecord simInfo) { //(SIMInfo simInfo) {

        Log.d(TAG, "getIconDrawableBySimInfoRecord commd = ");
        int simInfoColor = -1;
        if (simInfo != null) {
    ...
    public String getIccPhotoUriString(Bundle args) {
        ...
        Log.d(TAG, "getIccPhotoUriString commd = ");
        String photoUri = null;

        boolean isSdn = args.getBoolean(DefaultIccCardExtension.KEY_IS_ICC_CONTACT_SDN);
        int colorId = args.getInt(DefaultIccCardExtension.KEY_ICC_COLOR_ID); // color//slot
    ...
    public Drawable getIccPhotoDrawable(Bundle args) {
        ...
        long photoId = args.getLong(DefaultIccCardExtension.KEY_PHOTO_ID);

        if (photoId == 0) {
            Uri uri = args.getParcelable(DefaultIccCardExtension.KEY_PHOTO_URI);
            photoId = getPhotoIdByPhotoUri(uri);
        }
        boolean darkTheme = args.getBoolean(DefaultIccCardExtension.KEY_IS_DARK_THEME);
        int id = (int) photoId;
        int res = 0;
