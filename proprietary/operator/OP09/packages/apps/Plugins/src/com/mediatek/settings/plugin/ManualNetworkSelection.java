...
public class ManualNetworkSelection extends PreferenceActivity implements OnSwitchNetworkListener,
    ...
    private static final String KEY_CDMA_NETWORK_SELECTION_DETAIL =
            "cdma_network_selectioin_detail";
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)
                ...
                log("ACTION_PS_DETACH_DONE: slotId = " + slotId + " mWaitDataDisconnected = " +
                        mWaitDataDisconnected);
                if (mWaitDataDisconnected) {
                    ...
                    mDataDetachHandler.sendEmptyMessageDelayed(MSG_DATA_DETACHED,
                            DATA_DETACHED_DELAY);
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            if ((mPhoneSwitchingMode == SWITCH_PROCESS_SWITCH_PHONE_DONE)
                ...
                if (regState == ServiceState.REGISTRATION_STATE_HOME_NETWORK
                    ...
                    mPhoneSwitchingMode = SWITCH_PROCESS_DONE;
                    if (isG2C) {
                        // In CDMA mode, show alert dialog directly when registered network
                        // duration switch phone.
                        showDialog(DIALOG_SWITCH_NETWORK_REGISTERED);
                        mCdmaNetworkSelectionDetailPref.updateNetworkSearchingState(false);
                    } else {
                        // Do not show reg alert dialog here, or GSM network loading dialog will
                        // cover it. Delay to show this alert after GSM network load finish
            ...
            if (regState == ServiceState.REGISTRATION_STATE_HOME_NETWORK
                ...
                if (mTargetSlot == PhoneConstants.GEMINI_SIM_1) {
                    ...
                    // TODO: This API is not check in, mark it to resolve build error
    ...
    private void setTitleAndInitPreferences() {
        ...
        mCdmaNetworkSelectionDetailPref = (CdmaNetworkSwitcherPreference) findPreference(
                KEY_CDMA_NETWORK_SELECTION_DETAIL);
    ...
    private void updateScreen() {
        ...
        if (isRadioOn && !isAirplanMode) {
            if (mTargetSlot == PhoneConstants.GEMINI_SIM_1) {
                ...
                mCdmaNetworkSelectionPref.setEnabled(externalSlotInRoaming);
                ...
                if (phone != null) {
                    if (phone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                        ...
                        boolean isRegistered = (phone.getServiceState().getRegState() ==
                                ...
                                || phone.getServiceState().getRegState() ==
                                ServiceState.REGISTRATION_STATE_ROAMING);
            ...
            if (mTelephonyManager.getCallState() != TelephonyManager.CALL_STATE_IDLE) {
                mCdmaNetworkSelectionPref.setEnabled(false);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mCdmaNetworkSelectionPref) {
            ...
            if (Utils.isGeminiSupport()) {
                ...
                // network.
                if (mTargetSlot == PhoneConstants.GEMINI_SIM_1) {
                    log("[IRC] onPreferenceTreeClick blockSwitchPhone for network selecton " +
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        switch (id) {
            ...
            case DIALOG_SWITCH_CDMA_NETWORK_FAILED_DATA_CONNECT:
                dialog = builder.setMessage(getResources().getString(
                        R.string.manual_network_switch_cdma_fail_data_connect))
                        .setPositiveButton(android.R.string.ok, new OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
    ...
    private void loadGsmNetworksList() {
        ...
        if (Utils.isGeminiSupport() && !PhoneFactory.isDualTalkMode()) {
            if (mConnectivityManager.getMobileDataEnabled()) {
                ...
                log("loadGsmNetworksList, dataConnectionId = " + dataConnectionId +
                        " mTargetSlot = " + mTargetSlot);
    ...
    private void loadGsmNetworListImpl() {
        if (DBG) {
            log("loadGsmNetworListImpl, mWaitDataDisconnected = " + mWaitDataDisconnected +
                    " mTargetSlot = " + mTargetSlot);
        }
        if (mIsForeground) {
        ...
        if (!mWaitDataDisconnected) {
            ...
            mDataDetachHandler.sendEmptyMessageDelayed(MSG_DATA_DETACH_TIME_OUT,
                    DATA_DETACH_TIME_OUT_DELAY);
        }
    }

    private void networkQuery() {
        log("networkQuery, mWaitDataDisconnected = " + mWaitDataDisconnected +
                " mTargetSlot = " + mTargetSlot);
    ...
    /**
     * The message hander for network switching.
     */
                ...
                case MSG_SWITCH_NETWORK:
                    ...
                    if (null == ar.exception) {
                        ...
                        log("AvoidSysHandler handleMessage msg.what = " + MSG_SWITCH_NETWORK +
                            " mCdmaManuallyNetworkSelectState = " +
                                mCdmaManuallyNetworkSelectState);
                    } else {
                        showDialog(DIALOG_SWITCH_NETWORK_FAILED);
                    }
     ...
     * When user need to do GSM manual network selection, the other card data connection should
     * disable because of modem limitation.
    ...
    private void processCdmaSelectNextNetworkManually(ServiceState serviceState) {
        final int regState = serviceState.getRegState();
        log("processCdmaSelectNextNetworkManually, mCdmaManuallyNetworkSelectState = " +
            mCdmaManuallyNetworkSelectState
