...
import com.mediatek.internal.telephony.internationalroaming.InternationalRoamingConstants;
...
/**
 * for CT feature,when ACTION_BOOT_COMPLETED.
 * start this service for roaming changed,update date and time.
 */
public class RoamingService extends Service {
    ...
    private static final String TIME_ZONE_ACTIVITY_NAME =
        "com.mediatek.settings.plugin.TimeZoneActivity";
    ...
    public void onDestroy() {
        Log.d(TAG, "onDestroy");
        mTelephonyManagerEx.listen(mPhoneStateListener,
                PhoneStateListener.LISTEN_NONE, mEVDOModemSlot);
    ...
    private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onServiceStateChanged(ServiceState serviceState) {
            ...
            /// @}
            Log.d(TAG, "onServiceStateChanged mSimStat=" + mSimStat);
            if (mSimStat == TelephonyManager.SIM_STATE_ABSENT) {
                Log.w(TAG, "Sim absent ..");
                return;
            }
            try {
                if (mITelephony != null) {
                ...
                Log.e(TAG, "error to getInternationalCardType ");
                return;
            }

            // Only works for dual-mode SIM card
            Log.d(TAG, "onServiceStateChanged mCardType=" + mCardType);
            if (mCardType != InternationalRoamingConstants.CARD_TYPE_DUAL_MODE) {
                Log.w(TAG, "Only works for dual-mode SIM card..return");
                return;
            }
            ...
            if (serviceState.getRegState() == ServiceState.REGISTRATION_STATE_HOME_NETWORK
                ...
                Log.v(TAG, "Registered on network...");
                mRegisteredNetwork = true;
                removeNoServiceDialog();
                String mccMnc = serviceState.getOperatorNumeric();
                Log.v(TAG, "serviceState.getOperatorNumeric(): " + mccMnc);
                if (mccMnc != null && mccMnc.length() >= 5 && !mccMnc.equals("00000")) {
                    String mcc = mccMnc.substring(0, 3);
                    String mnc = mccMnc.substring(3, 5);
                    Log.v(TAG, "serviceState.getOperatorNumeric() mcc: " + mcc + "mnc: " + mnc);
                    String latestMCCMNC =
                        PreferenceManager.getDefaultSharedPreferences(
                                RoamingService.this).getString("ct_latest_mccmnc", null);
                    // store the latest MCCMNC
                    PreferenceManager.getDefaultSharedPreferences(
                            RoamingService.this).edit().
                            putString("ct_latest_mccmnc", mccMnc).commit();
                    /*
                     * In international roaming status,
                     * if the device is registered to CDMA network after
                     * booting,"Time dispaly mode" dialogue should pop up automatically.
                     */
                    if (mIsFirstBoot &&
                            mCardType == PhoneConstants.PHONE_TYPE_CDMA &&
                            isInternationalRoamingStatus()) {
                        Log.v(TAG, "the device is registered to CDMA network...");
                        notifyTimeZoneRecommend(false);
                        mIsFirstBoot = false;
                        return;
                    }
                    if (latestMCCMNC != null && !isSupportNitz() && isGgStandBy()) {
                        Log.v(TAG, "Register g network and don't support Nitz...");
                        String latestMcc = latestMCCMNC.substring(0, 3);
                        String latestMnc = latestMCCMNC.substring(3, 5);
                        if (mccMnc.startsWith(latestMcc)) {
                            Log.v(TAG, "MCC isn't changed mcc: " + mcc);
                            // mcc isn't changed, and mnc changed,
                            // show time zone recommend notification
                            if (!mccMnc.endsWith(latestMnc)) {
                                showTimeZoneRecommendInNotificationBar();
                            }
                        } else {
                            Log.v(TAG, "MCC is changed, last mccmnc: " +
                                    latestMCCMNC + ", new mccmnc: " + mccMnc);
                            ...
                            if (isMultipltTimeZone(Integer.valueOf(mcc))) {
                                ...
                                // 2. pop up time choose dialog -->
                                // CT comment needn't pop up dialog here
                                MccTable.updateMccMncConfiguration(RoamingService.this, mccMnc);
                            }
                        }
                    }
                    Log.v(TAG, "Latest MCCMNC: " + latestMCCMNC + ", new mccmnc: " + mccMnc);
                }
            }
        }
    };
    ...
    private boolean isGgStandBy() {
        ...
        if (insertedSimList.size() >= 2) {
            ...
            // GG dual stand by
            Log.d(TAG, "GG dual stand by," + ",slot1PhoneType :" + slot1PhoneType);
            Log.d(TAG, "GG dual stand by," + ",slot2PhoneType :" + slot2PhoneType);
            if ((slot1PhoneType == PhoneConstants.PHONE_TYPE_GSM &&
                    slot2PhoneType == PhoneConstants.PHONE_TYPE_GSM)) {
                return true;
            }
        } else if (insertedSimList.size() == 1) {
            if (insertedSimList.get(0).mSlot == PhoneConstants.GEMINI_SIM_1) {
                int slot1PhoneType = mTelephonyManagerEx.getPhoneType(PhoneConstants.GEMINI_SIM_1);
                Log.d(TAG, "slot1PhoneType :" + slot1PhoneType);
                if (slot1PhoneType == PhoneConstants.PHONE_TYPE_GSM) { // G net and not support NITZ
                    return true;
                }
            } else {
                int slot2PhoneType = mTelephonyManagerEx.getPhoneType(PhoneConstants.GEMINI_SIM_2);
                Log.d(TAG, "slot2PhoneType :" + slot2PhoneType);
    ...
    private boolean isSupportNitz() {
        ...
        if (insertedSimList.size() >= 2) {
            ...
            Log.d(TAG, "GG dual stand by, sim1Nitz is :" + sim1Nitz);
            Log.d(TAG, "GG dual stand by, sim2Nitz is :" + sim2Nitz);
            if ((("".equals(sim1Nitz)) || ("".equals(sim1Nitz)))) {
                // Slot1 or Slot2 not support NITZ
                return false;
            }
        } else if (insertedSimList.size() == 1) {
            if (insertedSimList.get(0).mSlot == PhoneConstants.GEMINI_SIM_1) {
                String sim1Nitz = SystemProperties.get("gsm.nitz.time", "");
                // Add Slot1 as parameter
                Log.d(TAG, "sim1Nitz is :" + sim1Nitz);
                if ("".equals(sim1Nitz)) { // G net and not support NITZ
                    return false;
                }
            } else {
                String sim2Nitz = SystemProperties.get("gsm.nitz.time.2", "");
                // Add Slot2 as parameter
                Log.d(TAG, "sim2Nitz is :" + sim2Nitz);
                if ("".equals(sim2Nitz)) {
                    return false;
                }
            }
    ...
    private void showTimeZoneRecommendInNotificationBar() {
        ...
        Notification notification = new Notification(
                R.drawable.ic_notification_time_zone_recommendation,
                notificationTitle,
        ...
        if (getPackageManager().resolveActivity(backIntent, 0) != null) {
            ...
            Log.e(TAG, "Could not find OP09 plugin.TimeZoneActivity.");
        }
    ...
    private boolean isMultipltTimeZone(int mcc) {
        ...
        if (iso == null) {
            ...
            if (tz.length < 2) {
                Log.d(TAG, "isMultipltTimeZone false");
                return false;
            }
            Log.d(TAG, "isMultipltTimeZone true");
            return true;
        }
    }
    ...
    private boolean isInternationalRoamingStatus() {
        ...
        Log.d(TAG, "is International Roaming Status: " + isRoaming);
        return isRoaming;
    }
    ...
    private void removeNoServiceDialog() {
        if (mNoServiceDialog != null && mNoServiceDialog.isShowing()) {
            Log.d(TAG, "Remove No Service Dialog.");
