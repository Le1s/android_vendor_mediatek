...
/**
 * Choose APN activity when use two sim card.
 */
public class MultipleSimActivity extends PreferenceActivity
    ...
    private class MultipleSimReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action) ||
                ...
                Log.d(TAG, "MultipleSimReceiver " +
                        "Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action) start");
                updatePreferenceEnableState();
                Log.d(TAG, "MultipleSimReceiver " +
                        "Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action) end");
                ...
                List<SimInfoRecord> temp =
                    SimInfoManager.getInsertedSimInfoList(MultipleSimActivity.this);
                ...
                if (temp.size() <= SINGLE_SIM_CARD) {
                    ...
                    Log.d(TAG, "MultipleSimReceiver else start");
                    updatePreferenceEnableState();
                    Log.d(TAG, "MultipleSimReceiver else end");
                }
                // /@}
            }
        }
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            ...
            switch (state) {
                case TelephonyManager.CALL_STATE_IDLE:
                    Log.d(TAG, "mPhoneStateListener else start");
                    updatePreferenceEnableState();
                    Log.d(TAG, "mPhoneStateListener else end");
                    break;
                default:
                    break;
            }
    ...
    private void initPreferenceScreen() {
        ...
        for (int i = 0; i < mSimNumbers; ++i) {
            ...
            mPref2CardSlot.put(prefSet.getPreference(i),
                    Integer.valueOf(mSimList.get(i).mSimSlotId));
        }
    }
        ...
        for (int i = 0; i < prefSet.getPreferenceCount(); i++) {
            if ((preference == prefSet.getPreference(i)) && (mTargetClass != null)) {
                ...
                if (isRadioOn(slotId)) {
                    ...
                    intent.putExtra(SUB_TITLE_NAME,
                            SimInfoManager.getSimInfoBySlot(this, slotId).mDisplayName);
                    intent.setClassName(mTargetPackage, mTargetClass);
                    checkToStart(slotId, intent);
                }
            }
    ...
    private boolean isCdmaPhone() {
        ...
        if (sim0Type == PhoneConstants.PHONE_TYPE_CDMA ||
                sim1Type == PhoneConstants.PHONE_TYPE_CDMA) {
            return true;
        } else {
            return false;
        }
