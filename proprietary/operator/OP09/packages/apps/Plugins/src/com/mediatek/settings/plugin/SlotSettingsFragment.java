...
public class SlotSettingsFragment extends PreferenceFragment {
    ...
    private static final String INTENT_ACTION_START_SWITCH_PHONE =
            "com.mediatek.intent.action.START_RADIO_TECHNOLOGY";
    ...
    private void selectNetworkAutomatic() {
        ...
        if (!Utils.isGeminiSupport()) {
            ...
            CurrentNetworkInfoStatus.getPhoneBySlotId(mGeminiPhone, mTargetSlot)
                .setNetworkSelectionModeAutomatic(msg);
        }
        /// @}
    }
        ...
        switch (dualSimMode) {
            ...
            case SIM_SLOT_1_RADIO_ON:
                if (slot == PhoneConstants.GEMINI_SIM_1) {
                    ...
                    Xlog.d(TAG, "Turning off sim " + slot + " and all sim radio is off");
                } else if (slot == PhoneConstants.GEMINI_SIM_2) {
                    if (mIsSim1Inserted) {
                        dualState = ALL_RADIO_ON;
                        Xlog.d(TAG, "sim 0 was radio on and now turning on sim " + slot);
                    } else {
                        dualState = SIM_SLOT_2_RADIO_ON;
                        Xlog.d(TAG, "Turning on only sim " + slot);
                    }
    ...
    /**
     * @return If the service state of external slot is roaming.
     */
    public static boolean externalSlotInRoamingService() {
        String currentNetworkNumeric = SystemProperties.get(
                TelephonyProperties.PROPERTY_OPERATOR_NUMERIC, NUMERIC_UNKNOWN);
