...
function build_tee ()
{
    ...
    if [ "${MTK_TEE_SUPPORT}" = "yes" ] ; then
        if [ "${TRUSTONIC_TEE_SUPPORT}" = "yes" ] ; then
            ...
            ./build.sh ${TRUSTZONE_BUILD_MODE} ${TEE_RAW_IMAGE_NAME} ${TRUSTZONE_IMAGE_OUTPUT_PATH} ${TRUSTZONE_PROTECT_BSP_PATH} ${TRUSTZONE_PROTECT_PRIVATE_PATH} ${TRUSTZONE_COMMON_SRC_PATH} ${TRUSTZONE_SRC_RELEASE_PATH} ${TRUSTZONE_APP_INSTALL_PATH}
            if [ $? -ne 0 ]; then 
