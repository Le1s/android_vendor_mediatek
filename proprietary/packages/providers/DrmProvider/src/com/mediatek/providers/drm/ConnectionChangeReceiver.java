...
public class ConnectionChangeReceiver extends BroadcastReceiver {
    ...
    private static Thread sThread = null; // the thread is static so that only
                                          // one thread is running to do SNTP
                                          // sync
                ...
                while (startIndex < sHostList.length) {
                    ...
                    if (-1 != result) {
                        ...
                        if (oft != Ntp.INVALID_OFFSET) {
                            ...
                            if ("invalid".equals(secTime)
                                    || "invalid-need-synchronization".equals(secTime)
                                    || "".equals(secTime)) {
                                ...
                                if (Math.abs(sentSec + oft - iSecTime) > 60) {
                                    Log.d(TAG,
                                            "time&clock invalid, continue to sync with server.");
    ...
    private static String[] sHostList = new String[]{"hshh.org", "t1.hshh.org", "t2.hshh.org",
            "t3.hshh.org", "clock.via.net", "pool.ntp.org", "asia.pool.ntp.org",
