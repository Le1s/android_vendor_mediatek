...
public class CellBroadcastMainSettings extends PreferenceActivity
    ...
    void refreshEnableCheckBox() {
        ...
        if (subInfoRecordList == null || 
                (subInfoRecordList != null && subInfoRecordList.isEmpty())) {
            Log.d(TAG, "there is no sim card");
            canReceiveMessage = false;
        }
        for (SubInfoRecord subInfo : subInfoRecordList) {
            // check whether has a SIM that can receive CMAS
            if (TelephonyManagerEx.getDefault().getSimState(subInfo.slotId) == TelephonyManager.SIM_STATE_READY) {
