...
import com.hesine.nmsg.activity.AccountActivity;
import com.hesine.nmsg.activity.ConversationActivity;
import com.hesine.nmsg.activity.SystemSetting;
import com.hesine.nmsg.api.ImageLoader;
...
import com.hesine.nmsg.util.DateTool;
...
public class ConversationListAdapter extends BaseAdapter {
	...
	private int[] userAvadars = new int[] { R.drawable.avadar1,
			R.drawable.avadar2, R.drawable.avadar3, R.drawable.avadar4,
			R.drawable.avadar5, R.drawable.avadar6, R.drawable.avadar7,
			R.drawable.avadar8, R.drawable.avadar9 };// ,
	TextView mTime;
	TextView mType;
	TextView mMsgBody;
	ImageView mMsgPhoto;

	private final int NMSG_TEXT_FROM_VIEW = 0;
	private final int NMSG_IMAGE_FROM_VIEW = 1;
	private final int NMSG_AUDIO_FROM_VIEW = 2;
	private final int NMSG_TEXT_TO_VIEW = 3;
	private final int NMSG_IMAGE_TO_VIEW = 4;
	private final int NMSG_AUDIO_TO_VIEW = 5;
	private final int NMSG_VIDEO_FROM_VIEW = 6;
	private final int NMSG_SINGLE_VIEW = 7;
	private final int NMSG_MULTIPOLE_VIEW = 8;
	private final int NMS_READ_MODE_MAX = 9;
...
//				.getSystemService(Context.VIBRATOR_SERVICE);
	...
	private void initSomeSpecifics() {
		if (mScreenWidth == 0) {
			...
			mSingleMultipoleImageWith = (int) (mScreenWidth - 40 * (mContext
	...
	public String getTime(int position, long timeStamp) {
		boolean ret = false;
		MLog.trace(TAG, "getTime:" + position);
	...
	public String getAttachmentViaMsg(LocalMessageInfo msg, int index) {
		...
		int dispType = getItemViewTypeViaMsg(msg);
		if (dispType == NMSG_AUDIO_FROM_VIEW
				|| dispType == NMSG_IMAGE_FROM_VIEW
				|| dispType == NMSG_SINGLE_VIEW
				|| dispType == NMSG_MULTIPOLE_VIEW
				|| dispType == NMSG_VIDEO_FROM_VIEW) {
			attachment = AttachInfo.getAttachmentAbsPathByUrl(msg
					.getMessageItems().get(index).getAttachInfo());
			if (!FileEx.ifFileExisted(attachment)) {
				ImageInfo ii = new ImageInfo();
				ii.setPath(attachment);
				ii.setUrl(msg.getMessageItems().get(index).getAttachInfo()
						.getUrl());
				mImageLoader.request(ii);
			}
		} else {
			attachment = AttachInfo.getAttachmentAbsPath(msg.getMessageItems()
					.get(index).getAttachInfo());
		}
		return attachment;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			case NMSG_TEXT_FROM_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_from_text,
						null);
				holderFromText = new ViewHolderFromText();
				holderFromText.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderFromText.msgView = (TextView) convertView
						.findViewById(R.id.chat_msgcont);
				holderFromText.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
				convertView.setTag(holderFromText);
				break;
			case NMSG_TEXT_TO_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_to_text,
						null);
				holderToText = new ViewHolderToText();
				holderToText.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderToText.msgView = (TextView) convertView
						.findViewById(R.id.chat_msgcont);
				holderToText.msgStatus = (ImageView) convertView
						.findViewById(R.id.chat_status);
				holderToText.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
				convertView.setTag(holderToText);
				break;
			case NMSG_IMAGE_FROM_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_from_image,
						null);
				holderFromPic = new ViewHolderFromImage();
				holderFromPic.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderFromPic.msgPic = (ImageView) convertView
						.findViewById(R.id.chat_from_picture);
				holderFromPic.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
			...
			case NMSG_IMAGE_TO_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_to_image,
						null);
				holderToPic = new ViewHolderToImage();
				holderToPic.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderToPic.msgStatus = (ImageView) convertView
						.findViewById(R.id.chat_status);
				holderToPic.msgPic = (ImageView) convertView
						.findViewById(R.id.chat_to_picture);
				holderToPic.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
				holderToPic.imageFrame = (ImageView) convertView
						.findViewById(R.id.image_frame);
				convertView.setTag(holderToPic);
				break;
			case NMSG_VIDEO_FROM_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_from_video,
						null);
				holderFromVideo = new ViewHolderFromVideo();
				holderFromVideo.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderFromVideo.descView = (TextView) convertView
						.findViewById(R.id.chat_from_video_text);
				holderFromVideo.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
...
//						.findViewById(R.id.image_frame);
			...
			case NMSG_AUDIO_FROM_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_from_audio,
						null);
				holderFromAudio = new ViewHolderFromAudio();
				holderFromAudio.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				...
				holderFromAudio.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
				convertView.setTag(holderFromAudio);
				break;
			case NMSG_AUDIO_TO_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_to_audio,
						null);
				holderToAudio = new ViewHolderToAudio();
				holderToAudio.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				...
				holderToAudio.avatar = (ImageView) convertView
						.findViewById(R.id.avatar);
				convertView.setTag(holderToAudio);
				break;
			case NMSG_SINGLE_VIEW:
				convertView = mInflater.inflate(R.layout.chat_item_from_single,
						null);
				holderSingle = new ViewHolderSingle();
				holderSingle.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderSingle.msgPic = (ImageView) convertView
						.findViewById(R.id.image);
				holderSingle.subject = (TextView) convertView
						.findViewById(R.id.subject);
				holderSingle.summary = (TextView) convertView
			...
			case NMSG_MULTIPOLE_VIEW:
				convertView = mInflater.inflate(
						R.layout.chat_item_from_multipole, null);
				holderMultipole = new ViewHolderMultipole();
				holderMultipole.timeView = (TextView) convertView
						.findViewById(R.id.chat_time);
				holderMultipole.msgPic = (ImageView) convertView
						.findViewById(R.id.image);
				holderMultipole.subject = (TextView) convertView
						.findViewById(R.id.subject);
			...
			default:
				MLog.error(TAG, "unknow display type");
				break;
			}
		} else {
			try {
				...
				case NMSG_IMAGE_TO_VIEW:
					...
					break;
				case NMSG_VIDEO_FROM_VIEW:
					holderFromVideo = (ViewHolderFromVideo) convertView
							.getTag();
					break;
				case NMSG_MULTIPOLE_VIEW:
					holderMultipole = (ViewHolderMultipole) convertView
							.getTag();
					break;
				case NMSG_SINGLE_VIEW:
					...
					break;
				case NMSG_AUDIO_FROM_VIEW:
					holderFromAudio = (ViewHolderFromAudio) convertView
							.getTag();
					break;
				case NMSG_AUDIO_TO_VIEW:
		...
		case NMSG_IMAGE_FROM_VIEW:
			...
			setPicView(holderFromPic.msgPic, attachment,
					R.drawable.ic_lose_small, false, holderFromPic.imageFrame);
			setContactAvatar(holderFromPic.avatar, msg.getFrom());
			break;
		case NMSG_IMAGE_TO_VIEW:
			setTimeView(holderToPic.timeView, time);
			setPicView(holderToPic.msgPic, attachment,
					R.drawable.ic_lose_small, false, holderToPic.imageFrame);
		...
		case NMSG_MULTIPOLE_VIEW:
			...
			setPicView(holderMultipole.msgPic, attachment,
					R.drawable.ic_lose_big, true, null);
			holderMultipole.subject.setText(msg.getMessageItems().get(0)
					.getSubject());
			...
			for (int i = 1; i < msg.getMessageItems().size(); i++) {
				View v = mInflater.inflate(
						R.layout.chat_item_multipole_subitem, null);
				...
				item.subjectView.setText(msg.getMessageItems().get(i)
						.getSubject());
				attachment = getAttachmentViaMsg(msg, i);
				setPicView(item.msgPic, attachment, R.drawable.ic_lose_small,
						false, null);
				...
				v.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						((ConversationActivity) mContext).onItemClick(null, v,
								-1, -1);
					}
				});
				v.setOnLongClickListener(new OnLongClickListener() {
					...
					public boolean onLongClick(View v) {
						((ConversationActivity) mContext).onItemLongClick(null,
								v, -1, -1);
						return true;
					}
				});
		...
		case NMSG_SINGLE_VIEW:
			...
			setPicView(holderSingle.msgPic, attachment, R.drawable.ic_lose_big,
					true, null);
			holderSingle.subject.setText(msg.getMessageItems().get(0)
					.getSubject());
			holderSingle.summary
					.setText(msg.getMessageItems().get(0).getDesc());
			break;
		case NMSG_AUDIO_FROM_VIEW:
			setTimeView(holderFromAudio.timeView, time);
			setContactAvatar(holderFromAudio.avatar, msg.getFrom());
			if (mPlayRecordId != null
					&& mPlayRecordId
							.equalsIgnoreCase(msg.getMsgid().toString())) {
				holderFromAudio.playState.setImageResource(R.drawable.play);
			} else {
				holderFromAudio.playState
						.setImageResource(R.drawable.ic_play_01);
			}
			if (attachment.indexOf("_") != -1) {
				String attachmentName = msg.getMessageItems().get(0)
						.getAttachInfo().getName();
				String length = attachmentName.substring(0,
						attachmentName.indexOf("_"));
				String text = length + "'";
		...
		case NMSG_AUDIO_TO_VIEW:
			...
			if (msg.getMsgid().toString().equals(mPlayRecordId)) {
	...
	public void setPlayRecordId(String recordId) {
	...
	private void setContactAvatar(ImageView v, final String contactId) {
		...
		if (contactId.equals(mAccount)) {
			if (mAccountBitmap != null) {
				...
				Drawable avatar = mContext.getResources().getDrawable(
	...
	private void setStatusView(ImageView v, int drawable, LocalMessageInfo msg) {
		...
		if (drawable == 0) {
			...
			v.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					((ConversationActivity) mContext).getResendDialog().show()
							.setTag(v.getTag());
				}
			});
		}
	...
	private void setPicView(ImageView v, String path, int defaultDrawable,
			boolean isWide, ImageView frame) {
		...
		if (bitmap == null) {
			...
			if (bitmap != null) {
				if (isWide) {
					...
					int targetHeight = (bitmap.getHeight() * targetWidth)
							/ bitmap.getWidth();
					bitmap = ImageUtils.resizeImage(bitmap, targetWidth,
							targetHeight, true);
				} else {
					if (bitmap.getWidth() > mScreenWidth * 3 / 7) {
						...
						if (w > bitmap.getWidth()) {
							bitmap = ImageUtils
									.resizeImage(bitmap, bitmap.getWidth(),
											bitmap.getHeight(), true);
						} else {
							bitmap = ImageUtils.resizeImage(bitmap,
									mScreenWidth * 3 / 7,
									bitmap.getHeight() * (mScreenWidth * 3 / 7)
											/ bitmap.getWidth(), true);
						}
					} else if (bitmap.getWidth() > mScreenWidth * 2 / 7) {
						bitmap = ImageUtils.resizeImage(bitmap,
								mScreenWidth * 2 / 7,
								bitmap.getHeight() * (mScreenWidth * 2 / 7)
										/ bitmap.getWidth(), true);
					}
				}
	...
	private SpannableStringBuilder setSpan(CharSequence msg) {
		...
		if (s2 instanceof Spannable) {
			...
			for (URLSpan url : urls) {
				...
				style.setSpan(myURLSpan, s2.getSpanStart(url),
						s2.getSpanEnd(url), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
