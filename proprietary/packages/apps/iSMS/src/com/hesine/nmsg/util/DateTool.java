...
public class DateTool {
	...
	public static SimpleDateFormat SDF = getDateFormat(Application.getInstance());
	...
	public static String getCurrentFormatTime(long lateMillis) {
		...
		Date today = new Date(curDate.getYear(), curDate.getMonth(), curDate
				.getDate(), 0, 0, 0);
	...
	public static String getDispTimeStr(long time) {
		...
		if (now.getYear() == inDate.getYear()) {
			if (now.getMonth() == inDate.getMonth()
					&& now.getDate() == inDate.getDate()) {
				return SDF2.format(time);
			}
			bThisYear = true;
		}
		
		String fm = Settings.System.getString(Application.getInstance()
		...
		if(TextUtils.isEmpty(fm)) fm = getDefaultFM(bThisYear);
	...
	public static String getDispLoneTimeStr(long time) {
		SDF = getDateFormat(Application.getInstance());
