...
import com.hesine.nmsg.api.NmsgApiProvider;
...
//	private static final String TAG = "DBUtils";
	...
	public static Long insertServiceInfo(ServiceInfo si, PicInfo pi) {
    	...
    	if(!ifServiceAccountExisted(si.getAccount())) {
    		id = LocalStore.instance().replaceOrThrow(table, null, values);
            if (id >= 0) {
                statistics.getInstance().accountStatus(si.getAccount(), String.valueOf(1));
                NmsgApiProvider.accounts.add(si.getAccount());
                si.setIcon(PicInfo.getPicPath(pi));
                if(!CommonUtils.isExistSystemContactViaAccount(si)){
                    CommonUtils.addContactInPhonebook(si);
                }
            }
    	} else {
    		LocalStore.instance().update(table, values, null, null);
    	}
        return id;
    }
	
	public static ServiceInfo getServiceInfo(String serviceAccount) {
		String[] columns = { "_id", "account", "account_name", "email",  "phone_number", "desc", "update_time", "icon", "status", "is_exist"};
		String selection = "account = '" + serviceAccount + "'";
		String table = LocalStore.TABLE_ACCOUNTS;
		Cursor c = LocalStore.instance().query(table, columns, selection,
				null, null, null, null);
	...
	public static Long insertMessage(String account, Long threadId, Long smsId, MessageInfo info, int status) {
		Long id = null;
		ContentValues values = new ContentValues();
		values.put("account", account);
		values.put("thread_id", threadId);
		values.put("sms_id", smsId);
		values.put("msg_uuid", info.getMsgid());
		...
		if (iis != null && iis.size() > 0) {
			for (MessageItemInfo messageItemInfo : iis) {
				insertMessageItem(info.getMsgid(), messageItemInfo);
			}
		}
	...
	public static boolean updateMessageStatus(String msgId, int status_code) {
		boolean ret = false;
		ContentValues values = new ContentValues();
		values.put("status", status_code);
		String table = LocalStore.TABLE_MESSAGES;
		int update = LocalStore.instance().update(table, values, "msg_uuid = '" + msgId + "'", null);
		if (update > 0) {
			ret = true;
		}
		return ret;
        ...
        while(null != cr && cr.moveToNext()){
            if(cr.getCount() > 0){
                if(cr.getInt(cr.getColumnIndex(MSG_STATUS)) == LocalMessageInfo.STATUS_FROM_UNREAD){
                    updateMessageStatus(cr.getString(cr.getColumnIndex(MSG_UUID)), LocalMessageInfo.STATUS_FROM_READ);
                }
            }
        }
        if(null != cr){
	...
	public static Long insertMessageItem(String msgId, MessageItemInfo info) {
		Long id = null;
		ContentValues values = new ContentValues();
		values.put("msg_uuid", msgId);
	...
	public static Long insertUser(UserInfo info, String iconPath) {
		Long id = null;
		ContentValues values = new ContentValues();
	...
	public static Long updateUser(UserInfo info) {
		...
		values.put("update_time", System.currentTimeMillis());
		String table = LocalStore.TABLE_USERINFO;
		//id = (long) LocalStore.instance().update(table, values, "phone_number =?", new String[]{info.getPhoneNum()});
		id = (long) LocalStore.instance().update(table, values, "account =?", new String[]{info.getAccount()});
		if(id<=0){
			insertUser(info);
		}
		return id;
	...
	public static UserInfo getUser(String account) {
		String[] columns = { "_id", "account", "email", "user_name", "age", "phone_number", "sign", "user_sex", "icon", "update_time"};
		String table = LocalStore.TABLE_USERINFO;
	...
	public static Cursor getMessageCursor(String account) {
		String[] columns = { "_id", "account", "thread_id", "sms_id", "msg_uuid", "msg_type", "sender", "receiver", "sms", "status", "update_time" };
		String selection = "account = '" + account + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);
		return c;
	}	
	
	public static Cursor getMessageCursorViaMsgUuid(String uuid) {
		String[] columns = { "_id", "account", "thread_id", "sms_id", "msg_uuid", "msg_type", "sender", "receiver", "sms", "status", "update_time" };
		String selection = "MSG_UUID = '" + uuid + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);
		return c;
	}
	
	public static int getMessageStatusViaMsgId(String MsgId) {
		String[] columns = { "_id", "status" };
		String selection = "msg_uuid = '" + MsgId + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);
	...
	public static String getUuIdViaAccount(String Account) {
		String[] columns = { "_id", "min(msg_uuid)" };
		String selection = "account = '" + Account + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);
		...
		if(c != null && c.moveToFirst()) {
			if(c.getCount() > 0) {
				ret = c.getString(c.getColumnIndex("min(msg_uuid)"));
			}
		}	
	...
	public static int getMessageStatusViaSmsId(long smsId) {
		String[] columns = { "_id", "status" };
		String selection = "sms_id = '" + smsId + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);
	...
	public static int getUnreadNumViaAccount(String account) {		
		String[] columns = { "_id", "account", "thread_id", "sms_id", "msg_uuid", "msg_type", "sender", "receiver", "sms", "status", "update_time" };
		String selection = "account = '" + account + "' and status = '" + LocalMessageInfo.STATUS_FROM_UNREAD + "'";
		Cursor c = LocalStore.instance().query(LocalStore.TABLE_MESSAGES, columns, selection,
				null, null, null, null);		
		int ret = c.getCount();
	...
	public static Cursor getMessageItemCursor(String msgId) {
		String[] columns = { "_id", "msg_uuid", "item_id", "subject", "desc", "short_link", "long_link", "body", "attach_type", "attach_name", "attach_size", "attach_url", "attachment" };
		String selection = "msg_uuid = '" + msgId + "'";
	...
	public static LocalMessageInfo getMsgViaCursor(Cursor cursor) {
		...
		msg.setMsgid(cursor.getString(cursor.getColumnIndex(MSG_UUID)));
		...
		Cursor cursorItem = getMessageItemCursor(msg.getMsgid());
	...
	public static MessageItemInfo getMsgItemViaCursor(Cursor cursor) {
		...
		String attachType = cursor.getString(cursor
				.getColumnIndex(MSG_ITEM_ATTACH_TYPE));
	...
	public static void deleteMsgViaUUId(String msgUUID) {
		Long smsId = getSMSIDViaUUId(msgUUID);
		LocalStore.instance().delete(LocalStore.TABLE_MESSAGES, "msg_uuid = ?", new String[] { String.valueOf(msgUUID) });
		LocalStore.instance().delete(LocalStore.TABLE_MESSAGE_ITEM, "msg_uuid = ?", new String[] { String.valueOf(msgUUID) });
		NmsMtkBinderApi.getInstance().delete(Uri.parse("content://sms/" + smsId), null, null);
	}

	public static long getSMSIDViaUUId(String msgUUID) {
		String[] columns = { "_id", "sms_id" };
		String selection = "msg_uuid = '" + msgUUID + "'";
		String table = LocalStore.TABLE_MESSAGES;
		Cursor c = LocalStore.instance().query(table, columns, selection,
				null, null, null, null);
		long ret = -1;
