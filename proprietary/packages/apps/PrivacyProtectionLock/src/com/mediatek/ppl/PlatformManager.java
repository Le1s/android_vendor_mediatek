...
public class PlatformManager {
    ...
    protected void sendMultipartTextMessage(String destinationAddress, String scAddress, ArrayList<String> parts,
        ...
        Log.d(TAG, "sendMultipartTextMessage: simId = " + simId + ", subId = " + subId);

        SmsManager.getSmsManagerForSubscriber(subId).
            sendMultipartTextMessage(destinationAddress, scAddress, parts, sentIntents, null);
    }
   ...
   public static List<PplSimInfo> buildSimInfo(Context context) {
        ...
        //List<SubInfoRecord> simItem =  SimInfoManager.getInsertedSimInfoList(context);
        List<SubInfoRecord> simItem =  SubscriptionManager.getActiveSubInfoList();
        ...
        for (int i = 0; i < simItem.size(); i++) {
            PplSimInfo map = new PplSimInfo(simItem.get(i).displayName, 
                    simItem.get(i).simIconRes[0], simItem.get(i).color);

            Log.i(TAG, "mSimSlotId: " + simItem.get(i).slotId);
            Log.i(TAG, "mDisplayName: " + simItem.get(i).displayName);
            Log.i(TAG, "Image: " + simItem.get(i).simIconRes[0]);
    ...
    private static class SimInfoComparable implements Comparator<SubInfoRecord> {
        @Override
         public int compare(SubInfoRecord sim1, SubInfoRecord sim2) {
            return sim1.slotId - sim2.slotId;
         }
     }
