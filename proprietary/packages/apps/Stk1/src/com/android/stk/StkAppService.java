    ...
    public void onCreate() {
        ...
        for (i = 0; i < mSimCount; i++) {
            ...
            if (TelephonyManager.getDefault().hasIccCard(i)) {
                ...
                if (subId != null && SubscriptionManager.isValidSubId(subId[0])) {
        ...
        for (int i = 0; i < mSimCount; i++) {
            ...
            if (mPhoneStateChangeReg[i] == true && mPhone[i] != null) {
                if (i == PhoneConstants.SIM_ID_1) {
                    mPhone[i].unregisterForPreciseCallStateChanged(mCallHandler);
                    mPhone[i].unregisterForSuppServiceFailed(mCallHandler);
                } else if (i == PhoneConstants.SIM_ID_2) {
                    mPhone[i].unregisterForPreciseCallStateChanged(mCallHandler2);
                    mPhone[i].unregisterForSuppServiceFailed(mCallHandler2);
                } else if (i == PhoneConstants.SIM_ID_3) {
                    mPhone[i].unregisterForPreciseCallStateChanged(mCallHandler3);
                    mPhone[i].unregisterForSuppServiceFailed(mCallHandler3);
                } else {
                    mPhone[i].unregisterForPreciseCallStateChanged(mCallHandler4);
                    mPhone[i].unregisterForSuppServiceFailed(mCallHandler4);
                }
            } else {
        ...
        public void handleMessage(Message msg) {
            ...
            case OP_LAUNCH_APP:
                if (mStkContext[slotId].mMainCmd == null) {
                    ...
                    toast = Toast.makeText(mContext.getApplicationContext(), R.string.main_menu_not_initialized, Toast.LENGTH_LONG);
...
*/
                if (mStkContext[slotId].mAvailable != STK_AVAIL_AVAILABLE) {
                    toast = Toast.makeText(mContext.getApplicationContext(), R.string.lable_not_available, Toast.LENGTH_LONG);
                    toast.setGravity(Gravity.BOTTOM, 0, 0);
                    toast.show();
                    StkAppService.mIsLauncherAcceptInput = true;//SystemProperties.set(StkAppService.PROPERTY_LAUNCHER_ACCEPT_INPUT, "1");
                    return;
                ...
                //If there is a pending activity for the slod id,
            ...
            case OP_CMD:
                ...
                break;
            case OP_RESPONSE:
                CatLog.d(LOG_TAG, " [OP_RESPONSE][responseNeeded] : " + mStkContext[slotId].responseNeeded);
...
*/
    ...
    private void handleSessionEnd(int slotId) {
        ...
        //if (mStkContext[slotId].mMenuIsVisible) {
            CatLog.d(LOG_TAG, "[handleSessionEnd][mMenuState]" + mStkContext[slotId].mMenuState);
        ...
        if (false == SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
            ...
            if (fg != null) {
                ...
                if (tmpPreCallState != Call.State.DISCONNECTED && Call.State.DISCONNECTED == state) {
                    CatLog.d(LOG_TAG, "Send OP_EVDL_CALL_DISCONN_TIMEOUT");                
...
*/
    ...
    private boolean canShowTextDialog(TextMessage msg, int slotId) {
        ...
        CatLog.d(LOG_TAG, "canShowTextDialog? mMenuIsVisible = " + mStkContext[slotId].mMenuIsVisible + " mInputIsVisible = " + " mDialogIsVisible = " + mStkContext[slotId].mDialogIsVisible);
    ...
    public boolean isIdleScreen(Context context) {
        ...
        CatLog.d(LOG_TAG, "[isIdleScreen] homePackage is: " + homePackage);
        ...
        public void onReceive(Context context, Intent intent) {
            if (false == SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
                ...
                if (evtAction.equals("android.intent.action.stk.CALL_DISCONNECTED")) {
                    CatLog.d(LOG_TAG, "mEventDownloadCallReceiver() - Received[CALL_DISCONNECTED]");
                    simId = intent.getIntExtra("slotId", PhoneConstants.SIM_ID_1);                
                } else {
                    CatLog.d(LOG_TAG, "mEventDownloadCallReceiver() - Received needn't handle!");
                    return;
                }
                ...
                try {
                    ...
                    CatLog.d(LOG_TAG, "mStkService is null, sim: " + simId);
                }
            }
        }
    };
                ...
                for (i = 0; i < mSimCount; i++) {
                    ...
                    if (i >= PhoneConstants.SIM_ID_1 && i < mSimCount) {
                        ...
                        if (mPhoneStateChangeReg[i] == false) {
                            if (i == PhoneConstants.SIM_ID_1) {
                                mPhone[i].registerForPreciseCallStateChanged(mCallHandler,
                                        PHONE_STATE_CHANGED, null);
                                mPhone[i].registerForSuppServiceFailed(mCallHandler,
                                        SUPP_SERVICE_FAILED, null);
                            } else if (i == PhoneConstants.SIM_ID_2) {
                                mPhone[i].registerForPreciseCallStateChanged(mCallHandler2,
                                        PHONE_STATE_CHANGED, null);
                                mPhone[i].registerForSuppServiceFailed(mCallHandler2,
                                        SUPP_SERVICE_FAILED, null);
                            } else if (i == PhoneConstants.SIM_ID_3) {
                                mPhone[i].registerForPreciseCallStateChanged(mCallHandler3,
                                        PHONE_STATE_CHANGED, null);
                                mPhone[i].registerForSuppServiceFailed(mCallHandler3,
                                        SUPP_SERVICE_FAILED, null);
                            } else {
                                mPhone[i].registerForPreciseCallStateChanged(mCallHandler4,
                                        PHONE_STATE_CHANGED, null);
                                mPhone[i].registerForSuppServiceFailed(mCallHandler4,
                                        SUPP_SERVICE_FAILED, null);
                            }
                            mPhoneStateChangeReg[i] = true;
                        }
                    }
                }
            }
    ...
    private final BroadcastReceiver mStkCallReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (evtAction.equals(ACTION_STK_CALL)) {
                long subId = intent.getLongExtra(EXTRA_STK_CALL_SUBID, 0);
        ...
        for (simIdx = PhoneConstants.SIM_ID_1; simIdx < mSimCount; simIdx++) {
            ...
            if (ICCID_STRING_FOR_NO_SIM.equals(mIccId[simIdx])) {
                ...
                if ((mIccId[simIdx] != null) || false == ("".equals(mIccId[simIdx]))) {
                    ...
                    appInstaller.install(mContext, simIdx);
