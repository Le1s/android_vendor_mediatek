...
public class IpMessageManager extends MessageManager {
    ...
    public IpMessage getIpMsgInfo(long msgId) {
        ...
        if (ipMessage != null) {
            ...
            Logger.e(TAG,
                    "getIpMsgInfo() cannot find this message, lets reload from db");
			Cursor cursor = null;
			try {
				ContentResolver contentResolver = AndroidFactory
						.getApplicationContext().getContentResolver();

				String[] args = { Long.toString(msgId) };
				...
				if (cursor.moveToFirst()) {
                    ...
                    messageList.add(id);
                    Logger.v(TAG,
                            "getIpMsgInfo() send reload request to ModelImpl" + "messageList"
                            		+ messageList);
                    PluginController.obtainMessage(
                            ChatController.EVENT_RELOAD_MESSAGE, messageList)
                            .sendToTarget();
				} else {
	                Logger.d(TAG, "getIpMsgInfo() cursor is empty");
	            }
			}catch (Exception e) {
				Logger.e(TAG, "getIpMsgInfo() exception");
	            e.printStackTrace();
	        }
			finally {
				if (null != cursor) {
