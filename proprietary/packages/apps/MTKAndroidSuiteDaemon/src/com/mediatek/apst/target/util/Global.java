...
public abstract class Global {
        ...
        if (null != sContext) {
            ...
            if (null != info) {
                if (Config.MTK_GEMINI_SUPPORT) {
                    sourceLocation = info.slotId + 1;
                } else {
                    sourceLocation = info.slotId;
                }
            }
    ...
    public static SimDetailInfo getSimInfoById(int simId) {
        ...
        if (null != sContext) {
            ...
            if (null != info) {
                ...
                deailInfo.setSlotId((int) info.subId);
                if (Config.MTK_GEMINI_SUPPORT) {
                    deailInfo.setAccessible(SystemInfoProxy
                            .getSimAccessibleBySlot((int) info.subId));
                }
            } else {
            ...
            try {
                if (Config.MTK_GEMINI_SUPPORT) { // Here need not modification
                                                 // for 4 SIM.
                    if ("USIM".equals(iTel.getIccCardType(info.subId))) {
    ...
    public static SimDetailInfo getSimInfoBySlot(int slotId) {
        ...
        if (null != sContext) {
            List<SubInfoRecord> subInfoList = SubscriptionManager.getSubInfoUsingSlotId(slotId);
            if (subInfoList != null && subInfoList.size() > 0) {
                SubInfoRecord info = subInfoList.get(0);
                if (null != info) {
                    deailInfo.setColor(info.color);
                    deailInfo.setDisplayName(info.displayName);
                    deailInfo.setNumber(info.number);
                ...
                if (USIMUtils.isSimUsimType((int) info.subId)) {
                    deailInfo.setSIMType(SimDetailInfo.SIM_TYPE_USIM);
                } else {
                    deailInfo.setSIMType(SimDetailInfo.SIM_TYPE_SIM);
                }
    ...
    public static int getSimIdBySlot(int slotId) {
        ...
        if (null != sContext) {
            SubInfoRecord info = SubscriptionManager.getSubInfoForSubscriber(slotId);
            if (null != info) {
                Debugger.logD(new Object[] { slotId }, "The simId is " + (int) info.subId);
                return (int) info.subId;
            }
        } else {
            Debugger.logW("mContext is null");
        }
    ...
    public static List<SimDetailInfo> getAllSIMList() {
        if (null != sContext) {
            ...
            List<SubInfoRecord> list = SubscriptionManager.getAllSubInfoList();
            if (null != list) {
                int size = list.size();
                // detailInfoList = new ArrayList<SimDetailInfo>();
                for (int i = 0; i < size; i++) {
                    SubInfoRecord info = list.get(i);
                    SimDetailInfo deailInfo = new SimDetailInfo();
                    if (null != info) {
                        deailInfo.setColor(info.color);
                        deailInfo.setDisplayName(info.displayName);
