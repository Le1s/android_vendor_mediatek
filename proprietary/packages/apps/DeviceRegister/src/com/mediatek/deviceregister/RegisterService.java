...
public class RegisterService extends Service {
    ...
    private boolean isUimAvailable(int uimId) {
        ...
        if (mTelephonyManager.hasIccCard(uimId)) {
            long[] subId = SubscriptionManager.getSubId(uimId);
            ...
            if (TelephonyManager.PHONE_TYPE_CDMA == phoneType) {
                ...
                String netwOperator = mTelephonyManager.getNetworkOperator(subId[0]);
    ...
    public String getCurrentLogonImsi() {
        ...
        if (mTelephonyManager != null) {
            ...
            if (currentUim != Const.UIM_NONE) {
                long[] subId = SubscriptionManager.getSubId(currentUim);
                if (subId == null || subId[0] == -1) {
    ...
    private String getEsnFromUim() {
        ...
        ITelephony iTel = ITelephony.Stub.asInterface(
                ServiceManager.getService(Context.TELEPHONY_SERVICE));
        try {
            byte[] readResult = iTel.iccExchangeSimIOExUsingSlot(0, fileId, readCommand, 0, 0,
                    sEsnLength, path, null, null);
