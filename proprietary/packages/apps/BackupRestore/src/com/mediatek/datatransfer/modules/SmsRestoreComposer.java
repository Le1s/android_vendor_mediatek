    ...
    private ContentValues parsePdu(SmsRestoreEntry pdu) {
        ...
        if (Utils.isGeminiSupport()) {
            ...
            if (soltid < 0) {
                ...
                List<SubInfoRecord> list = SubscriptionManager.getSubInfoUsingSlotId(soltid);
                if (list != null && list.size() > 0) {
                    SubInfoRecord si = list.get(0);
                    if (si == null) {
                        simCardid = "-1";
                    } else {
                        simCardid = String.valueOf(si.subId);
                    }
                } else {
                    simCardid = "-1";
                }
        ...
        values.put(Sms.SUB_ID, simCardid);
