...
public class DeviceManagerActivity extends PreferenceActivity {
    ...
    protected void onStop() {
        super.onStop();
        Log.i(TAG, "[onStop]...");
        mCachedBluetoothLEDevice.unregisterAttributeChangeListener(mAttributeListener);
    ...
    protected void onStart() {
        ...
        this.mAnpServiceConnected = mLocalBluetoothLEManager
                .getServiceConnectionState(LocalBleManager.PROFILE_ANP_ID);
        Log.i(TAG, "[onStart] mAnpServiceConnected : " + mAnpServiceConnected);

        initActivity();
        initActionBar();
        mCachedBluetoothLEDevice.registerAttributeChangeListener(mAttributeListener);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mLocalBluetoothLEManager.getCurrentState() == BluetoothAdapter.STATE_ON) {
            if (mCachedBluetoothLEDevice != null) {
                if (mCachedBluetoothLEDevice.getConnectionState() == BluetoothGatt.STATE_CONNECTED) {
                    menu.add(0, MENU_DISCONNECT_FLAG, 0, R.string.disconnect_text);
                }
                if (mCachedBluetoothLEDevice.getConnectionState() == BluetoothGatt.STATE_DISCONNECTED) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case MENU_CONNECT_FLAG:
            final BluetoothDevice device = mCachedBluetoothLEDevice.getDevice();
            if (device == null) {
                Log.i(TAG, "[onOptionsItemSelected]device is null,break.");
                break;
            }
            mConnectAction.connect(device, mCachedBluetoothLEDevice.getDeviceLocationIndex(), true);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            case CROP_IMAGE:
                ...
                bitmap = ActivityUtils.saveImageFromCustom(this, cropUri, mCachedBluetoothLEDevice
            ...
            if (bitmap != null) {
                mCachedBluetoothLEDevice.updateDeviceImage(ActivityUtils
    ...
    private void initActivity() {
        ...
        mDeviceNameTextView.setText(mCachedBluetoothLEDevice.getDeviceName());
        ...
        mDeviceImageView.setImageURI(mCachedBluetoothLEDevice.getDeviceImageUri());
    ...
    private void initActionBar() {
        ...
        bar.setTitle(mCachedBluetoothLEDevice.getDeviceName());
    }
            ...
            public void onClick(DialogInterface arg0, int arg1) {
                if (editor.getText().toString().trim().length() != 0) {
                    if (!editor.getText().toString().equals(befText)) {
                        mCachedBluetoothLEDevice.setDeviceName(editor.getText().toString());
                        updateUi();
                        arg0.dismiss();
                    }
                }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case DELETE_DEVICE:
                BluetoothDevice device = mCachedBluetoothLEDevice.getDevice();
                if (device != null) {
                    Log.d(TAG, "[mHandler] DELETE_DEVICE, start to disconnect gatt device");
                    mLocalBluetoothLEManager.disconnectGattDevice(device);
                    device.removeBond();
                    mCachedBluetoothLEDeviceManager.removeDevice(mCachedBluetoothLEDevice);
