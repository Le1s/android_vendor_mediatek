    ...
    switch (par1)
    {
        ...
        case GET_AUDIO_POLICY_VOL_FROM_VER1_DATA:
        {
            ...
            if ((pTarget->bRev == CUSTOM_VOLUME_REV_1) && (len == sizeof(AUDIO_CUSTOM_VOLUME_STRUCT)))
            {
                ...
                memcpy((void*)pTarget->audiovolume_steamtype[CUSTOM_VOL_TYPE_TTS],(void*)Source.audiovolume_media, sizeof(unsigned char)*NUM_OF_VOL_MODE*AUDIO_MAX_VOLUME_STEP);
    ...
    do
    {
        ...
        else if (sources[0].type == AUDIO_PORT_TYPE_DEVICE)
        {
            ...
            if (sinks[0].type == AUDIO_PORT_TYPE_MIX)
            {
                ...
                status = mAudioMTKStreamManager->setParameters(param.toString(), sinks[0].ext.mix.handle);
                ...
                else if (eInputDeviceList == AUDIO_DEVICE_IN_FM_TUNER)
                {
                    if (mUseAudioPatchForFm == true)
                    {
                        ...
                        AudioFMController::GetInstance()->SetFmVolume(1.0);//For be sure mFmVolume doesn't equal to -1.0
...
*/                    
                    else // FM earphone
                    {
                        ...
                        status = mAudioMTKStreamManager->setParametersToStreamOut(param.toString());//The sources[0].ext.mix.handle doesn't record IOport
                        ...
                        {
                            #ifndef MTK_AUDIO_GAIN_TABLE //TODO: Gain Table case?
                            float fFMVolume = AudioMTKVolumeController::linearToLog(dGainDB);
                            ...
                            AudioFMController::GetInstance()->SetFmVolume(fFMVolume);//initial value is -1 , should change it first
                            #endif
                            // FM enable
    ...
    if (status == NO_ERROR)
    {
        ...
        *handle = android_atomic_inc(&mNextUniqueId);
        ...
        ALOGD("%s sucess new *handle 0x%x",__FUNCTION__, (int)(*handle));
    }
    else
    {
...
int AudioMTKHardware::releaseAudioPatch(audio_patch_handle_t handle)
{
    ...
    do
    {
        ...
        if (patch->sources[0].type == AUDIO_PORT_TYPE_MIX)
        {
            ...
            status = mAudioMTKStreamManager->setParameters(param.toString(), patch->sources[0].ext.mix.handle);
#endif
        }
        else if (patch->sources[0].type == AUDIO_PORT_TYPE_DEVICE)
        {
            ...
            if (patch->sinks[0].type == AUDIO_PORT_TYPE_MIX)
            {
                ...
                if (mUseAudioPatchForFm == true)
                    {
                        if (patch->sources[0].ext.device.type == AUDIO_DEVICE_IN_FM_TUNER)
                        {
                                for (index = 0; index < mAudioHalPatchVector.size(); index++)
                                {
                                    ...
                                    {
                                        if (mAudioHalPatchVector[index]->sources[source_i].type == AUDIO_PORT_TYPE_DEVICE && mAudioHalPatchVector[index]->sources[source_i].ext.device.type == AUDIO_DEVICE_IN_FM_TUNER)
                                        {
                                                ALOGD("Still have AudioPatches need  AUDIO_DEVICE_IN_FM_TUNER, Don't Disable FM [%d]",mAudioHalPatchVector.size());
                                                status = NO_ERROR;
                                                bReturnFlag = true;
                                                break;
                                        }
                ...
                for (index = 0; index < mAudioHalPatchVector.size(); index++)
                {
                    for (int source_i=0;source_i<mAudioHalPatchVector[index]->num_sources;source_i++)
                    {
                        if (mAudioHalPatchVector[index]->sources[source_i].type == AUDIO_PORT_TYPE_DEVICE && mAudioHalPatchVector[index]->sources[source_i].ext.device.type != AUDIO_DEVICE_NONE)
                        {
                                eInDeviceList = mAudioHalPatchVector[index]->sources[source_i].ext.device.type;
                                ALOGD("Still have AudioPatches need  routing input device, Don't change routing [%d]",mAudioHalPatchVector.size());
                                status = NO_ERROR;
                                //bReturnFlag = true;
                                break;
                        }
                ...
                status = mAudioMTKStreamManager->setParameters(param.toString(), patch->sinks[0].ext.mix.handle);
            }
...
//                        bReturnFlag = false;
                        ...
                        for (index = 0; index < mAudioHalPatchVector.size(); index++)
                        {
                            for (int sink_i=0;sink_i<mAudioHalPatchVector[index]->num_sinks;sink_i++)
                            {
                                if (mAudioHalPatchVector[index]->sinks[sink_i].type == AUDIO_PORT_TYPE_DEVICE && mAudioHalPatchVector[index]->sinks[sink_i].ext.device.type != AUDIO_DEVICE_NONE)
                                {
                                        eOutDeviceList = mAudioHalPatchVector[index]->sinks[sink_i].ext.device.type;
                                        ALOGD("Still have AudioPatches routing to outputDevice, Don't routing null Size %d , device 0x%x",mAudioHalPatchVector.size(),eOutDeviceList);
                                        status = NO_ERROR;
//                                        bReturnFlag = true;
                                        break;
                                }
