...
status_t AudioALSAPlaybackHandlerBase::setLowLatencyMode(bool mode, size_t lowLatencyHalBufferSize, size_t reduceInterruptSize, bool bforce)
{
    return NO_ERROR;
}
...
status_t AudioALSAPlaybackHandlerBase::getHardwareBufferInfo(time_info_struct_t *HWBuffer_Time_Info)
{
    ...
    if (ret != 0)
    {
        ALOGV("-%s pcm_get_htimestamp fail, ret = %d, pcm_get_error = %s", __FUNCTION__, ret, pcm_get_error(mPcm));
