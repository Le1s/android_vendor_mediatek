    ...
    uint32_t flags)
{
    ...
    *status = NO_ERROR;
    ...
    if (*channels == AUDIO_CHANNEL_OUT_5POINT1 ||
        *channels == AUDIO_CHANNEL_OUT_7POINT1)
    {
        mStreamAttributeSource.buffer_size = 3072 * 4 * 2 * mStreamAttributeSource.num_channels * size_per_channel;
        mStreamAttributeSource.latency = (3072 * 4 * 2 * mStreamAttributeSource.num_channels * size_per_channel * 1000) / (mStreamAttributeSource.sample_rate * size_per_frame);
    }
    else
    {
    ...
    {
        mStreamAttributeSource.buffer_size = 8192; // TODO(Harvey): Query this
        mStreamAttributeSource.latency = (MAX_KERNEL_BUFFER_SIZE * 1000) / (mStreamAttributeSource.sample_rate * size_per_frame);
    }
#endif
...
#else
        mPlaybackHandler->setLowLatencyMode(mode != 0, 0, 8192);
#endif
    }
    return NO_ERROR;
}
