...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    m_camPass1Param.isp_top_ctl.CAM_CTL_SEL_GLOBAL.Bits.PASS1_DB_EN = 1;
    m_camPass1Param.isp_top_ctl.CAM_CTL_SEL_GLOBAL.Bits.PASS1_D_DB_EN = 1;
...
//        m_camPass1Param.imgo.format = portInfo_imgo.eImgFmt;
    ...
    if (-1 != idx_rrzo ) { // use output dma crop
        ...
        PIPE_DBG("[3ASync]rrzo crop=%d,%d",m_camPass1Param.rrzo.crop.w, m_camPass1Param.rrzo.crop.h );
...
*
********************************************************************************/
#define _CmdQSwitchEn(OpenedPort,mag_rrz0,mag_aa,mag_imgo)({\
    MBOOL _flag = MFALSE;\
    ...
    else if(OpenedPort == CAM_CTL_EN_P1_DMA_RRZO_EN){\
        if(mag_rrz0 == mag_aa)\
            _flag = MTRUE;\
    }\
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_RRZ:
            ...
            if ( EPortIndex_TG1I == arg2){
                ...
                if (_CmdQSwitchEn(this->mOpenedPort,mMagicNum0_D,mMagicNum1_D,mMagicNum2_D)) {
                    m_CamPathPass1.BurstQNum = mDupCmdQMgr.CmdQMgr_GetBurstQ();
                    m_CamPathPass1.DupQIdx = mDupCmdQMgr.CmdQMgr_GetDuqQIdx();
                    ...
                    if(this->m_bBypassImgo == MFALSE){
                        if(this->mOpenedPort & CAM_CTL_EN_P1_DMA_IMGO_EN){
                            if(0 != m_CamPathPass1.setP1ImgoCfg(this->m_ImgoCfg)){
                                PIPE_ERR("[3ASync]SET_RRZ D setP1imgoCfg not ready");
                                //ret = -1;
                            }
                        }
                    }
