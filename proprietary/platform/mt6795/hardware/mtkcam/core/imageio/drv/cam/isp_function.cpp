...
#define ISP_FUNC_AST(cond, fmt, arg...)  do { if (function_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)
    ...
    static volatile MUINT8 dma_array[_rt_dma_max_] = {0};//for fw rcnt controller
#endif
        ...
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NBC){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                ...
                if(regsStruct[i].Addr == CAM_ANR_CON1_OFFSET){  // debug ANR_SCALE_MODE be writed incorrectly
                    ISP_FUNC_INF("[CQ][0x%x,0x%x]",regsStruct[i].Addr,regsStruct[i].Data);
                }
...
int DMAO_B::_config( void )
{
    ...
    switch(this->id())
    {
        ...
        case ISP_DMA_IMGO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON2,0x00F0F000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x800000F0,ISP_DRV_USER_ISPF); // ultra-
            ...
            if(this->m_fps > FPS_BOUNDARY){
#if SRAM_SWITCH
                ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x810000E0,ISP_DRV_USER_ISPF); // ultra-
#endif
        ...
        case ISP_DMA_RRZO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x800000C0,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(this->m_pIspDrvShell->m_pPhyIspDrv_bak,CAM_IMGI_SLOW_DOWN,0x0c000000,ISP_DRV_USER_ISPF); // ultra-

            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON2,0x00c0c000,ISP_DRV_USER_ISPF);
...
#else
                ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x81000080,ISP_DRV_USER_ISPF); // ultra-
#endif
                {//special case to reduce kernel log
                    MUINT32 p1_log[6] = {0};
                    if(this->m_bKDBG){
        ...
        case ISP_DMA_IMGO_D:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON,0x800000F0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON2,0x00F0F000,ISP_DRV_USER_ISPF);
        ...
        case ISP_DMA_RRZO_D:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON,0x800000c0,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON2,0x00c0c000,ISP_DRV_USER_ISPF);
        ...
        case ISP_DMA_CAMSV_IMGO:
            ...
            //enlarge untra
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_CON,0x08000040,ISP_DRV_USER_ISPF); // ultra- 0x08404040
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_CON2,0x00404000,ISP_DRV_USER_ISPF); // 0x00404000
        ...
        case ISP_DMA_CAMSV2_IMGO:
            ...
            //enlarge ultra
            //ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON,0x08404040,ISP_DRV_USER_ISPF);   // ultra- 0x08404040
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON,0x08000040,ISP_DRV_USER_ISPF);   // ultra- 0x08404040
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON2,0x00404000,ISP_DRV_USER_ISPF);  //0x00404000
...
#define CAM_CTL_SEL_GLOBAL_P2_MASK  0x00006000
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    if(logPath == ISP_PASS2) { // P2
        ...
        ISP_FUNC_INF("[P2]en(0x%08x),EnDma(0x%08x),ctlSel(0x%08x),intEn(0x%08x),fmtSel(0x%08x),cqEn(0x%08x),global(0x%x)", \
    ...
    if ( ISP_PASS1   == this->path || ISP_PASS1_D == this->pathD ) {
           ...
           switch ( path[ii] ) {
               case ISP_PASS1:
                   ...
                   ISP_FUNC_INF("[PIP][TopCtrl]ISP_PASS1 P1:EN(0x%x/0x%x),DMA(0x%x/0x%x),fmt(0x%x/0x%x),sel(0x%x/0x%x),BYP(0x%x/0x%x),GLO(0x%x/0x%x/0x%x)\n", \
                    ...
                    #ifdef _rtbc_buf_que_2_0_
                    {
                        ...
                        ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,_rrzo_,0,0,(unsigned char*)&dma_array[0]};
                        this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
                    }
                    #endif
                   break;
               case ISP_PASS1_D:
                   ...
                   #ifdef _rtbc_buf_que_2_0_
                   {
                       ...
                       ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,_rrzo_d_,0,0,(unsigned char*)&dma_array[0]};
                       this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
                   }
                   #endif
        ...
        //inform kernel which dma ch r opened
             ...
             ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)this->CQ,0,0,(unsigned char*)&dma_array[0]};
             this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
        #endif
...
RUN_P2_CFG:
    ...
    if ((this->path==ISP_PASS2)||(this->path==ISP_PASS2B)||(this->path==ISP_PASS2C))
    {
        ...
        ISP_WRITE_REG(m_pP2IspDrv,CAM_CTL_SEL_GLOBAL_SET,(this->isp_top_ctl.CAM_CTL_SEL_GLOBAL.Raw&CAM_CTL_SEL_GLOBAL_P2_MASK),ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pP2IspDrv,CAM_CTL_SEL_GLOBAL_CLR,((~this->isp_top_ctl.CAM_CTL_SEL_GLOBAL.Raw)&CAM_CTL_SEL_GLOBAL_P2_MASK),ISP_DRV_USER_ISPF);
        #else
        //default, mfb_mode in 15004040 should be off
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if ( CAM_ISP_PASS1_START == (*start) ) {
        ...
        if(this->isP2IspOnly){
            ...
            irqPass2Done.Clear= ISP_DRV_IRQ_CLEAR_WAIT;
        ...
        #if 0 //kk test default:0
            //if((this->drvScenario == NSImageio::NSIspio::eDrvScenario_CC_SShot)){
            if(1){
                ISP_FUNC_INF("kk:cqpa(0x%x),tpipe(0x%x)",
                    gpMdpMgrCfgData[this->burstQueIdx][ISP_DRV_P2_CQ2][this->dupCqIdx].ispTpipeCfgInfo.drvinfo.DesCqPa,
                    gpMdpMgrCfgData[this->burstQueIdx][ISP_DRV_P2_CQ2][this->dupCqIdx].ispTpipeCfgInfo.drvinfo.tpipeTablePa);

                this->m_pIspDrvShell->m_pPhyIspDrv_bak->dumpCQTable(ISP_DRV_CQ02, this->burstQueIdx, this->dupCqIdx);
            }
        #endif
    }
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    if(ISP_PASS1 == this->path)
    {
        //replace gPass1Lock by gP1StopLock is to prevent blocking case between deque/stop. (e.g.:p1 deque will be blocked when p1_d r going to stop)
        //the functionality of gp1stoplock is the same as mlock in rawCentralCtrl theoratically, so need don't need to change
        _localVar = &this->m_pIspDrvShell->gP1StopLock;//_localVar = &this->m_pIspDrvShell->gPass1Lock;
    }
    else if(ISP_PASS1_D == this->pathD || ISP_PASS1_CAMSV == this->path || ISP_PASS1_CAMSV_D == this->pathD){
        //the functionality of gp1stoplock is the same as mlock in rawCentralCtrl theoratically, so need don't need to change
        _localVar = &this->m_pIspDrvShell->gP1StopLock;//_localVar = &this->m_pIspDrvShell->gPass1Lock;//&this->m_pIspDrvShell->gPass1_dLock; // avoid stop have racing issue under multi-sensor(2nd stop use reset when 1st stop r during fbc workaround rst)
    }
    else {
        _localVar = &this->m_pIspDrvShell->gPass2Lock;
    }
...
///////////////////S remove later after fbc ECO of ROME E2
    ...
    TdriDrvCfg* pSrcImageio)
{
    ...
    if(isApplyTuning==MTRUE){
        ...
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NBC_EN ){
            ...
            #endif
            ISP_FUNC_INF("(tpipe)anr_scale_mode(%d)",pDst->nbc.anr_scale_mode);
        }
...
int ISP_PASS1_CTRL::setP1RrzCfg( IspRrzCfg const rrz_cfg )
{
    ...
    if ( rrz_cfg.isTwinMode) {
        ...
        if(rrz_cfg.rrz_in_roi.floatX != 0){
            ISP_FUNC_ERR("rrz in floatx support only 0");
        }
        //support central crop only.(abitrary crop r not supported on ROME twin mode)
        if((rrz_cfg.rrz_in_roi.x*2 + rrz_cfg.rrz_in_roi.w) > rrz_cfg.rrz_in_size.w){
            ISP_FUNC_ERR("rrz input crop out of range: (x,y,floatx,floatxy,w,h = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x),(size:0x%x,0x%x)",\
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if ( _imgo_ == rt_dma || \
            ...
            for(MUINT32 i=0;i<count;i++) {
                ISP_QUE_INF(rt_dma,"i(%d),dma(%d),id(0x%x),size(0x%x),xsize(0x%x),VA(0x%llx),PA(0x%x),count(%d),cur sof(%d),frm_cnt(%d),mag(0x%x),rawType(%d)",
                    i, \
                    rt_dma, \
                ...
                bufInfo->dma_crop.x= GetCropXUnitPixel(deque_buf.data[i].dmaoCrop.x, bufInfo->img_fmt,deque_buf.data[i].image.bus_size);
        ...
        while(count--) {
            ...
            if (bufSize) {
                ...
                switch(rt_dma){
                    case _imgi_:
                        ISP_FUNC_INF("[imgi]drvSce(%d),rt_dma(%d)",this->drvScenario,rt_dma);
                        if(this->drvScenario != NSImageio::NSIspio::eDrvScenario_IP){
                ...
                if(isDequeMdp==MTRUE){
                    ...
                    if(mdpRet == MDPMGR_NO_ERROR){
                            ...
                            (rt_dma==_wrot_))
                        {
                                ...
                                #if 0
                                do{
                                    reg_4000 = ISP_READ_REG_NOPROTECT(mpPhyIspDrv, CAM_CTL_START);
                                }while(reg_4000!=0x00);
                                #endif
                                //
                        ...
                        //        this->m_pIspDrvShell->m_pPhyIspDrv_bak->dumpCQTable((ISP_DRV_CQ_ENUM)0,0, this->dequeDupCqIdx);
                                ...
                                this->m_pIspDrvShell->gPass2Lock.lock();
                                if((pWorkingBuffer!=NULL)&&(pIspReg!=NULL)) {
                        //kk test            tpipe_main_platform(&gpMdpMgrCfgData[this->dequeBurstQueIdx][p2Cq][this->dequeDupCqIdx].ispTpipeCfgInfo, &tpipeDesc, pWorkingBuffer, tpipeWorkingSize, (void*)pIspReg);
                                    delete pWorkingBuffer;
                                    pWorkingBuffer = NULL;
                                } else {
                                    ISP_FUNC_ERR("[Error]pWorkingBuffer(0x%08x),pIspReg(0x%08x)",pWorkingBuffer,pIspReg);
