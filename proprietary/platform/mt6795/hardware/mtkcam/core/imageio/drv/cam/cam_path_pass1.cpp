...
int CamPathPass1::config( struct CamPathPass1Parameter* p_parameter )
{
    ...
    if(p_parameter->path == ISP_PASS1)
        ...
        //_localVar = &this->ispTopCtrl.m_pIspDrvShell->gPass1Lock;//&this->ispTopCtrl.m_pIspDrvShell->gPass1_dLock; // avoid top_config,whcih in config2, have racing issue under multi-sensor(pass1 && pass1d config at the same time)
        //p1_d use gPass1Lock will block the configpipe of sub when main still in streaming.(block by main_deque)
        //using gPass1Lock is for the global var:rawCentralCtrl, should use the mutex in rawCentralCtrl to protect rawCentralCtrl.
        //and the mutex is used in ISP_TOP_CTRL::_config( void )
...
int CamPathPass1::_waitIrq( int type, unsigned int irq , int userNumber)
{
    ...
    if(CAM_CTL_INT_P1_STATUS_VS1_INT_ST == irq)
        WaitIrq.bDumpReg = 1;
    else
