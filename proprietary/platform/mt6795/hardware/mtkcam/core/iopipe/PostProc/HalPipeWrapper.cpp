...
checkCQEnqueuedUserNum(
    MINT32 p2cq)
{
    MINT32 maxNum=MAX_ENQUEUE_USER_PERCQ;
    if(p2cq==2)
    {
        maxNum=MAX_VSS_ENQUEUE_USER_PERCQ;
    }

    getLock(ELockEnum_CQUser);
    if(mp2CQUserNum[p2cq]==maxNum)
    {
...
startVideoRecord(MINT32 wd,MINT32 ht,MINT32 fps, ESoftwareScenario swScen)
{
	...
	if(mpPostProcPipe)
	{
		...
		//send information to venc if fhd@120 / hd@180
		if( (fps >=180 || (fps>=120 &&  ((wd*ht)>=(1920*1080)) )))
		{
			ret=mpPostProcPipe->startVideoRecord(wd,ht,fps,eDrv,cqIdx);
			mEnStartVideoRecord=true;
			if(!ret)
			{
				LOG_ERR("startVideo Record Fail");
			}
		}
		//add cmd to adjust thread priority for dequeue thread in slow motion
		sendCommand(EPathCQ_CQ1,ECmd_ADJTHPRI);
	}
	else
	{
