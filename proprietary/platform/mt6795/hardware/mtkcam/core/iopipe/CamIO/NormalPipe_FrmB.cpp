    ...
    if (1 == sensorCnt) {//NPIPE_Sensor_0 ||  NPIPE_Sensor_1
        ...
        if (NULL == pNPipe)  {
            ...
            if (pNPipe->mpFrmB_Thread == NULL){
                pNPipe->mpFrmB_Thread = NormalPipe_FrmB_Thread::CreateInstance((MUINTPTR)pNPipe);
                if(pNPipe->mpFrmB_Thread == NULL){
...
NormalPipe_FrmB::stop()
{
    ...
    if (this->mpCamIOPipe)
    {
        ...
        if(mTwinEnable[CAM_TG_1][0] == (unsigned long)this){
            ...
            PIPE_ERR("error:fail search in sensorIdx:%d(0x%x: 0x%x_0x%x_0x%x_0x%x)",this->mpSensorIdx[0],(unsigned long)this,\
            mTwinEnable[CAM_TG_1][0],mTwinEnable[CAM_TG_2][0],mTwinEnable[CAM_SV_1][0],mTwinEnable[CAM_SV_2][0]);
        }
    }
    else{
        PIPE_ERR("error:fail stop, NULL ptr for camiopipe\n");
    }
...
NormalPipe_FrmB::enque(QBufInfo const& rQBuf)
{
    ...
    if(this->m_bStarted == MFALSE){
        ...
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            rQBufInfo.vBufInfo[0].u4BufVA[0] =     rQBuf.mvOut.at(i).mBuffer->getBufVA(0); //
            rQBufInfo.vBufInfo[0].u4BufPA[0] =     rQBuf.mvOut.at(i).mBuffer->getBufPA(0); //
#else
            rQBufInfo.vBufInfo[0].u4BufSize[0] =   (MUINT32)rQBuf.mvOut.at(i).mSize;
            rQBufInfo.vBufInfo[0].u4BufVA[0] =     rQBuf.mvOut.at(i).mVa;
            rQBufInfo.vBufInfo[0].u4BufPA[0] =     rQBuf.mvOut.at(i).mPa;
#endif
...
#if 0
{
    ...
    if(this->mOpenedPort & NormalPipe_FrmB::_IMGO_ENABLE)
        ...
        this->mpDummyRequest[_PortMap(PORT_RRZO.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_getsize,(MUINT32)&_size,0);
...
NormalPipe_FrmB::_setTgPixelAndDualMode(void)
{
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        switch(tg){
            case CAM_TG_1:
                if ( (NSCam::SENSOR_TYPE_RAW  == _NORMALPIPE_GET_SENSORTYPE(SIdx)) ||
                    ...
                    this->mbTwoPixelEnable = ((_NORMALPIPE_GET_PIX_MODE(SIdx)) == TWO_PIXEL_MODE ? 1:0);
                }
                break;
            case CAM_TG_2:
                this->mbTwoPixelEnable = ((_NORMALPIPE_GET_PIX_MODE(SIdx)) == TWO_PIXEL_MODE ? 1:0);
        ...
        switch(tg){
            ...
            case CAM_SV_1:
            case CAM_SV_2:
                mTwinEnable[tg][1] = ((this->mbTwoPixelEnable == MTRUE)?ePxlMode_Two_Twin : ePxlMode_One_Single);
                break;
            default:
                PIPE_ERR("error: err tg=0x%x",tg);
                return MFALSE;
        ...
        PIPE_DBG("1sensor tg[%d],PixelMode=[%d],twin:0x%x(0x%x)", tg, _NORMALPIPE_GET_PIX_MODE(SIdx),this->mbTwoPixelEnable,mTwinEnable[tg][1]);
    }
...
NormalPipe_FrmB::configPipe(QInitParam const& vInPorts)
{
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++)  {
        ...
        //enforce seninf/tg in 2-pixmode if rrzo over 3072 is required
        for (int j=0; j<vInPorts.mPortInfo.size();j++) {
            if (( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID )|| ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID )) {
                if(vInPorts.mPortInfo.at(j).mDstSize.w > 3072){
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++) {
        ...
        for (int j=0; j<vInPorts.mPortInfo.size();j++) {
            if (( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID )|| ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID )) {
                if(vInPorts.mPortInfo.at(j).mDstSize.w > 3072){
                    PIPE_INF("configPipe Enable TwinMode due to RRZOut.W > 3072!!!!!(%d)\n",_NORMALPIPE_GET_TG_IDX((mpSensorIdx[0])));
         ...
         //MW must guarantee no rrzo > 3072 && 2nd raw sensor is needed.
         if(_b2pixEn){
              for (int j=0; j<vInPorts.mPortInfo.size();j++) {
                  if(_twinPort == PORT_RRZO){
                       if ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID ) {
                            PIPE_ERR("isp support no rrzo.w > 3072 && rrzo_d enable\n");
                            _rst = MFALSE;
                            break;
                       }
                  }else{//_twinport is PORT_RRZO_D
                       if ( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID ) {
                            PIPE_ERR("isp support no rrzo_d.w > 3072 && rrzo enable\n");
                            _rst = MFALSE;
                            break;
                       }
                  }
    ...
    if (!mpCamIOPipe)
    {
        ...
        for (int i=0; i<vInPorts.mPortInfo.size();i++) {
            ...
            pOut->ePxlMode  = (this->mbTwoPixelEnable == 1) ? (NSImageio::NSIspio::ePxlMode_Two_Twin):(NSImageio::NSIspio::ePxlMode_One_Single);

            if ((pOut->index  == EPortIndex_RRZO)||(pOut->index  == EPortIndex_RRZO_D)) {
                if ( (0 == this->mbTwoPixelEnable) && (/*CAM_RRZ_MAX_LINE_BUFFER_IN_PIXEL*/3072 < pOut->u4ImgWidth) ) {
                    PIPE_ERR("rrzo out width exceed (%d > 3072)",pOut->u4ImgWidth);
            ...
            PIPE_DBG("Crop:%d,%d,%d,%d", pOut->crop1.x, pOut->crop1.y,pOut->crop1.w,pOut->crop1.h);
        ...
        for (int i = 0; i < this->mpEnabledSensorCnt; i++)
        {
            switch ( _NORMALPIPE_GET_TG_IDX((this->mpSensorIdx[i])) )
            {
               case CAM_TG_1:
                   mEnablePath |= NPIPE_PASS_PASS1;
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_CHANNEL     ,(MINTPTR)EPIPE_PASS1_CQ0,0,0);
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0,(MINTPTR)EPIPECQ_TRIGGER_SINGLE_IMMEDIATE,(MINTPTR)EPIPECQ_TRIG_BY_START);
                   PIPE_DBG("CAM_TG_1: SetCQ0_Channel&Mode");
                   break;
               case CAM_TG_2:
                   mEnablePath |= NPIPE_PASS_PASS1_D;
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_D_CQ_CHANNEL     ,(MINTPTR)EPIPE_PASS1_CQ0_D,0,0);
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_D_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0_D,(MINTPTR)EPIPECQ_TRIGGER_SINGLE_IMMEDIATE,(MINTPTR)EPIPECQ_TRIG_BY_START);
        ...
        for (int i = 0; i < mpEnabledSensorCnt; i++)
        {
            switch ( _NORMALPIPE_GET_TG_IDX((mpSensorIdx[i])) )
            {
               case CAM_TG_1:
                   ...
                   //continuous mode
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0,(MINTPTR)EPIPECQ_TRIGGER_CONTINUOUS_EVENT,(MINTPTR)EPIPECQ_TRIG_BY_PASS1_DONE);
                   #else
                   //cause CQ0 was configurred as SINGLE_IMMEDIATELY
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0,(MINTPTR)EPIPECQ_TRIGGER_SINGLE_EVENT,(MINTPTR)EPIPECQ_TRIG_BY_PASS1_DONE);
               ...
               case CAM_TG_2:
                   ...
                   #if PASS1_CQ_CONTINUOUS_MODE__
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0_D,(MINTPTR)EPIPECQ_TRIGGER_CONTINUOUS_EVENT,(MINTPTR)EPIPECQ_TRIG_BY_PASS1_DONE);
                   #else
                   this->mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_TRIGGER_MODE,(MINTPTR)EPIPE_PASS1_CQ0_D,(MINTPTR)EPIPECQ_TRIGGER_SINGLE_EVENT,(MINTPTR)EPIPECQ_TRIG_BY_PASS1_DONE);
    ...
    {
        CamManager* pCamMgr = CamManager::getInstance();
        pCamMgr->setAvailableHint((0 == this->mbTwoPixelEnable)? MTRUE : MFALSE);
    }
...
*
*****************************************************/
NormalPipe_FrmB_Thread::NormalPipe_FrmB_Thread(MUINTPTR obj)
{
    m_bStart = MFALSE;
    mCnt = 0;
...
NormalPipe_FrmB_Thread* NormalPipe_FrmB_Thread::CreateInstance(MUINTPTR obj)
{
    if(obj == 0)
    {
...
MBOOL NormalPipe_FrmB_Thread::Enque(BufInfo* pBuf)
{
    ...
    if(_buf.mPa == pBuf->mBuffer->getBufPA(0)){
        ...
        rQBufInfo.vBufInfo[0].u4BufVA[0] =     pBuf->mBuffer->getBufVA(0); //
        rQBufInfo.vBufInfo[0].u4BufPA[0] =     pBuf->mBuffer->getBufPA(0); //
#else
        rQBufInfo.vBufInfo[0].u4BufSize[0] =   (MUINT32)pBuf->mSize;
        rQBufInfo.vBufInfo[0].u4BufVA[0] =     pBuf->mVa;
        rQBufInfo.vBufInfo[0].u4BufPA[0] =     pBuf->mPa;
#endif
        ...
        rQBufInfo.vBufInfo[0].u4BufVA[0] =     _buf.mVa;
        rQBufInfo.vBufInfo[0].u4BufPA[0] =     _buf.mPa;
#else
        rQBufInfo.vBufInfo[0].u4BufSize[0] =   (MUINT32)_buf.mSize;
        rQBufInfo.vBufInfo[0].u4BufVA[0] =     _buf.mVa;
        rQBufInfo.vBufInfo[0].u4BufPA[0] =     _buf.mPa;
        ...
        rQBufInfo.vBufInfo[1].u4BufVA[0] =     pBuf->mBuffer->getBufVA(0); //
        rQBufInfo.vBufInfo[1].u4BufPA[0] =     pBuf->mBuffer->getBufPA(0); //
#else
        rQBufInfo.vBufInfo[1].u4BufSize[0] =   (MUINT32)pBuf->mSize;
        rQBufInfo.vBufInfo[1].u4BufVA[0] =     pBuf->mVa;
        rQBufInfo.vBufInfo[1].u4BufPA[0] =     pBuf->mPa;
#endif
