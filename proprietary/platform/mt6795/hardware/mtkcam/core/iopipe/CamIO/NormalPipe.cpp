...
NormalPipe::stop()
{
    ...
    if (this->mpCamIOPipe)
    {
        ...
        if(mTwinEnable[CAM_TG_1][0] == (unsigned long)this){
            ...
            CAM_LOGE("error:fail search in sensorIdx:%d(0x%x: 0x%x_0x%x_0x%x_0x%x)",this->mpSensorIdx[0],(unsigned long)this,\
        ...
        CAM_LOGE("error:fail stop, NULL ptr for camiopipe\n");
    }
...
replace(BufInfo const& bufOld, BufInfo const& bufNew)
{
    ...
    rQBufInfo.vBufInfo[0].u4BufVA[0] =     bufOld.mBuffer->getBufVA(0); //
    rQBufInfo.vBufInfo[0].u4BufPA[0] =     bufOld.mBuffer->getBufPA(0); //
#else
    rQBufInfo.vBufInfo[0].u4BufSize[0] =   (MUINT32)bufOld.mSize;
    rQBufInfo.vBufInfo[0].u4BufVA[0] =     bufOld.mVa;
    rQBufInfo.vBufInfo[0].u4BufPA[0] =     bufOld.mPa;
#endif
    ...
    rQBufInfo.vBufInfo[1].u4BufVA[0] =     bufNew.mBuffer->getBufVA(0); //
    rQBufInfo.vBufInfo[1].u4BufPA[0] =     bufNew.mBuffer->getBufPA(0); //
#else
    rQBufInfo.vBufInfo[1].u4BufSize[0] =   (MUINT32)bufNew.mSize;
    rQBufInfo.vBufInfo[1].u4BufVA[0] =     bufNew.mVa;
    rQBufInfo.vBufInfo[1].u4BufPA[0] =     bufNew.mPa;
#endif
...
NormalPipe::_setTgPixelAndDualMode(void)
{
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        ...
        CAM_LOGD("1sensor tg[%d],PixelMode=[%d],twin:0x%x(0x%x)", tg, _NORMALPIPE_GET_PIX_MODE(SIdx),this->mbTwoPixelEnable,mTwinEnable[tg][1]);
    }
...
NormalPipe::configPipe(QInitParam const& vInPorts)
{
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++) {
        ...
        for (int j=0; j<vInPorts.mPortInfo.size();j++) {
            if (( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID )|| ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID )) {
                if(vInPorts.mPortInfo.at(j).mDstSize.w > 3072){
                    CAM_LOGI("configPipe Enable TwinMode due to RRZOut.W > 3072!!!!!(%d)\n",_NORMALPIPE_GET_TG_IDX((mpSensorIdx[0])));
              ...
              for (int j=0; j<vInPorts.mPortInfo.size();j++) {
                  if(_twinPort == PORT_RRZO){
                       if ( PORT_RRZO_D == vInPorts.mPortInfo.at(j).mPortID ) {
                            CAM_LOGE("isp support no rrzo.w > 3072 && rrzo_d enable\n");
                            _rst = MFALSE;
                            break;
                       }
                  }else{//_twinport is PORT_RRZO_D
                       if ( PORT_RRZO == vInPorts.mPortInfo.at(j).mPortID ) {
                            CAM_LOGE("isp support no rrzo_d.w > 3072 && rrzo enable\n");
                            _rst = MFALSE;
                            break;
                       }
                  }
    ...
    if (!mpCamIOPipe)
    {
        ...
        for (int i = 0; i < mpEnabledSensorCnt;i++)
        {
            ...
            if (1) {
               ...
               CAM_LOGD("SensorTg=%d, tgIdx=%d, devID=%d, eRawPxlID=%d,PixelMode=%d W/H=[%d, %d]",\
    ...
    //update cq table num if mw member update burst queue number
        ...
        for (int i=0; i<vInPorts.mPortInfo.size();i++) {
            ...
            if ((pOut->index  == EPortIndex_RRZO)||(pOut->index  == EPortIndex_RRZO_D)) {
                if ( (0 == mbTwoPixelEnable) && (/*CAM_RRZ_MAX_LINE_BUFFER_IN_PIXEL*/3072 < pOut->u4ImgWidth) ) {
                    CAM_LOGE("rrzo out width exceed (%d > 3072)",pOut->u4ImgWidth);
            ...
            CAM_LOGD("Crop:%d,%d,%d,%d", pOut->crop1.x, pOut->crop1.y,pOut->crop1.w,pOut->crop1.h);
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    if ( !mConfigDone ) {
        CAM_LOGE("error:sendCommand(0x%x) fail, send before configpipe",cmd);
        return MFALSE;
    }
