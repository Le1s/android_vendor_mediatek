...
int FlashlightDrv::isChargerIn(int* bCharger)
{
    logI("isChargerIn()\n");
	StrobeGlobalDriver* pKDrv;
	pKDrv = StrobeGlobalDriver::getInstance();
	int err;
	*bCharger = -1;
	err = pKDrv->sendCommandRet(FLASH_IOC_IS_CHARGER_IN, mSensorDev, mStrobeId, bCharger);
	logI("isChargerIn()=%d", *bCharger);
	return err;
}
int FlashlightDrv::isOtgUsing(int* bOtg)
{
    logI("isOtgUsing()\n");
	StrobeGlobalDriver* pKDrv;
	pKDrv = StrobeGlobalDriver::getInstance();
	int err;
	*bOtg = -1;
    err = pKDrv->sendCommandRet(FLASH_IOC_IS_OTG_USE, mSensorDev, mStrobeId, bOtg);
	logI("isOtgUsing()=%d", *bOtg);
	return err;
}

int FlashlightDrv::isHw6332(int* b6332)
{
    logI("isHw6332()\n");
	StrobeGlobalDriver* pKDrv;
	pKDrv = StrobeGlobalDriver::getInstance();
	...
	int flashDrvId;
	*b6332=0;
    err = pKDrv->sendCommandRet(FLASH_IOC_GET_FLASH_DRIVER_NAME_ID, mSensorDev, mStrobeId, &flashDrvId); 
	if(flashDrvId==(int)e_FLASH_DRIVER_6332)
	    *b6332=1;
    else
        *b6332=0;
	logI("isHw6332()=%d", *b6332);
	return err;
}
