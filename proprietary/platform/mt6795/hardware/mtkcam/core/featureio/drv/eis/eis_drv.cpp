...
EisDrvImp::EisDrvImp(const MUINT32 &aSensorIdx) : EisDrv()
{
    ...
    mFeoDmaBuf_1.memID = -5;
    mFeoDmaBuf_1.useNoncache = 0;
    mFeoDmaBuf_1.virtAddr = mFeoDmaBuf_1.phyAddr = mFeoDmaBuf_1.size = 0;

    mFeoDmaBuf_2.memID = -5;
    mFeoDmaBuf_2.useNoncache = 0;
    mFeoDmaBuf_2.virtAddr = mFeoDmaBuf_2.phyAddr = mFeoDmaBuf_2.size = 0;

    mFeoDmaBuf_3.memID = -5;
    mFeoDmaBuf_3.useNoncache = 0;
    mFeoDmaBuf_3.virtAddr = mFeoDmaBuf_3.phyAddr = mFeoDmaBuf_3.size = 0;
...
MINT32 EisDrvImp::Uninit()
{
    ...
    if(mUsers == 0)
    {
        ...
        DestroyMemBuf(1,&mFeoDmaBuf_1);
        
        mFeoDmaBuf_1.memID = -5;
        mFeoDmaBuf_1.useNoncache = 0;
        mFeoDmaBuf_1.virtAddr = mFeoDmaBuf_1.phyAddr = mFeoDmaBuf_1.size = 0;

        DestroyMemBuf(1,&mFeoDmaBuf_2);
        
        mFeoDmaBuf_2.memID = -5;
        mFeoDmaBuf_2.useNoncache = 0;
        mFeoDmaBuf_2.virtAddr = mFeoDmaBuf_2.phyAddr = mFeoDmaBuf_2.size = 0;

        DestroyMemBuf(1,&mFeoDmaBuf_3);
...
{ 
    if(mFeoDmaBuf_1.memID == -5)
    {
        MUINT32 feoMemSize = MAX_FEO_SIZE * 56; // 56 bytes for each block
        
        CreateMemBuf(feoMemSize,1,&mFeoDmaBuf_1);                
        if(mFeoDmaBuf_1.virtAddr == 0 && mFeoDmaBuf_1.phyAddr == 0)
        {
            EIS_ERR("mFeoDmaBuf_1 create IMem fail");      
            return EIS_RETURN_MEMORY_ERROR;           
        }

        EIS_LOG("mFeoDmaBuf_1: ID(%d),size(%u),VA(0x%08x),PA(0x%08x),NonCache(%d)",mFeoDmaBuf_1.memID,
                                                                                   mFeoDmaBuf_1.size,
                                                                                   mFeoDmaBuf_1.virtAddr,
                                                                                   mFeoDmaBuf_1.phyAddr,
                                                                                   mFeoDmaBuf_1.useNoncache);
    }

    if(mFeoDmaBuf_2.memID == -5)
    {
        MUINT32 feoMemSize = MAX_FEO_SIZE * 56; // 56 bytes for each block
        
        CreateMemBuf(feoMemSize,1,&mFeoDmaBuf_2);                
        if(mFeoDmaBuf_2.virtAddr == 0 && mFeoDmaBuf_2.phyAddr == 0)
        {
            EIS_ERR("mFeoDmaBuf_2 create IMem fail");      
            return EIS_RETURN_MEMORY_ERROR;           
        }

        EIS_LOG("mFeoDmaBuf_2: ID(%d),size(%u),VA(0x%08x),PA(0x%08x),NonCache(%d)",mFeoDmaBuf_2.memID,
                                                                                   mFeoDmaBuf_2.size,
                                                                                   mFeoDmaBuf_2.virtAddr,
                                                                                   mFeoDmaBuf_2.phyAddr,
                                                                                   mFeoDmaBuf_2.useNoncache);
    }

    if(mFeoDmaBuf_3.memID == -5)
    {
        MUINT32 feoMemSize = MAX_FEO_SIZE * 56; // 56 bytes for each block
        
        CreateMemBuf(feoMemSize,1,&mFeoDmaBuf_3);                
        if(mFeoDmaBuf_3.virtAddr == 0 && mFeoDmaBuf_3.phyAddr == 0)
        {
            EIS_ERR("mFeoDmaBuf_3 create IMem fail");      
            return EIS_RETURN_MEMORY_ERROR;           
        }

        EIS_LOG("mFeoDmaBuf_3: ID(%d),size(%u),VA(0x%08x),PA(0x%08x),NonCache(%d)",mFeoDmaBuf_3.memID,
                                                                                   mFeoDmaBuf_3.size,
                                                                                   mFeoDmaBuf_3.virtAddr,
                                                                                   mFeoDmaBuf_3.phyAddr,
                                                                                   mFeoDmaBuf_3.useNoncache);
    }
    return EIS_RETURN_NO_ERROR;
}
...
MVOID EisDrvImp::GetFeoRegInfo(FEO_COFIG_DATA *aFeoRegInfo)
{
    ...
    if(g_debugDump > 0)
    {
        EIS_LOG("mFeoDmaIdx(%u)",mFeoDmaIdx);
    }
    
    switch(mFeoDmaIdx)
    {
        case 0:
            aFeoRegInfo->memInfo.memID       = mFeoDmaBuf_1.memID;
            aFeoRegInfo->memInfo.virtAddr    = mFeoDmaBuf_1.virtAddr;
            aFeoRegInfo->memInfo.phyAddr     = mFeoDmaBuf_1.phyAddr;
            aFeoRegInfo->memInfo.size        = mFeoDmaBuf_1.size;
            aFeoRegInfo->memInfo.bufSecu     = mFeoDmaBuf_1.bufSecu;
            aFeoRegInfo->memInfo.bufCohe     = mFeoDmaBuf_1.bufCohe;    
            aFeoRegInfo->memInfo.useNoncache = mFeoDmaBuf_1.useNoncache;
            break;
        case 1:
            aFeoRegInfo->memInfo.memID       = mFeoDmaBuf_2.memID;
            aFeoRegInfo->memInfo.virtAddr    = mFeoDmaBuf_2.virtAddr;
            aFeoRegInfo->memInfo.phyAddr     = mFeoDmaBuf_2.phyAddr;
            aFeoRegInfo->memInfo.size        = mFeoDmaBuf_2.size;
            aFeoRegInfo->memInfo.bufSecu     = mFeoDmaBuf_2.bufSecu;
            aFeoRegInfo->memInfo.bufCohe     = mFeoDmaBuf_2.bufCohe;    
            aFeoRegInfo->memInfo.useNoncache = mFeoDmaBuf_2.useNoncache;
            break;
        case 2:
            aFeoRegInfo->memInfo.memID       = mFeoDmaBuf_3.memID;
            aFeoRegInfo->memInfo.virtAddr    = mFeoDmaBuf_3.virtAddr;
            aFeoRegInfo->memInfo.phyAddr     = mFeoDmaBuf_3.phyAddr;
            aFeoRegInfo->memInfo.size        = mFeoDmaBuf_3.size;
            aFeoRegInfo->memInfo.bufSecu     = mFeoDmaBuf_3.bufSecu;
            aFeoRegInfo->memInfo.bufCohe     = mFeoDmaBuf_3.bufCohe;    
            aFeoRegInfo->memInfo.useNoncache = mFeoDmaBuf_3.useNoncache;
            break;
         default : EIS_ERR("wrong mFeoDmaIdx(%u)",mFeoDmaIdx);            
    }    
}
...
MVOID EisDrvImp::FlushMemory(const EIS_DMA_ENUM &aDma,const EIS_FLUSH_ENUM &aFlush)
{
    ...
    else if(aDma == EIS_DMA_FEO)
    {
        if(aFlush == EIS_FLUSH_HW)
        {
            switch(mFeoDmaIdx)
            {
                case 0 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH,&mFeoDmaBuf_1);
                    break;
                case 1 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH,&mFeoDmaBuf_2);
                    break;
                case 2 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH,&mFeoDmaBuf_3);
                    break;
                default : EIS_ERR("cmd(%d),wrong mFeoDmaIdx(%u)",EIS_FLUSH_HW,mFeoDmaIdx);
            }            
        }
        else if(aFlush == EIS_FLUSH_SW)
        {
            ...
            switch(mFeoDmaIdx)
            {
                case 0 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID,&mFeoDmaBuf_1);
                    break;
                case 1 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID,&mFeoDmaBuf_2);
                    break;
                case 2 : m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID,&mFeoDmaBuf_3);
                    break;
                default : EIS_ERR("cmd(%d),wrong mFeoDmaIdx(%u)",EIS_FLUSH_SW,mFeoDmaIdx);
            }
            ...
            // sava to FEO queue
            {
                ...
                switch(mFeoDmaIdx)
                {
                    case 0 : mFeoData.push(mFeoDmaBuf_1.virtAddr);                             
                        break;
                    case 1 : mFeoData.push(mFeoDmaBuf_2.virtAddr);                             
                        break;
                    case 2 : mFeoData.push(mFeoDmaBuf_3.virtAddr);                             
                        break;
                    default : EIS_ERR("cmd(%d),wrong mFeoDmaIdx(%u)",EIS_FLUSH_SW,mFeoDmaIdx);
                }                
            }            
            ...
            if(g_debugDump > 0)
            {
                EIS_LOG("ori:mFeoDmaIdx(%u)",mFeoDmaIdx);
                switch(mFeoDmaIdx)
                {
                    case 0 : EIS_LOG("feoVA(%p)",mFeoDmaBuf_1.virtAddr);
                        break;
                    case 1 : EIS_LOG("feoVA(%p)",mFeoDmaBuf_2.virtAddr);
                        break;
                    case 2 : EIS_LOG("feoVA(%p)",mFeoDmaBuf_3.virtAddr);
                        break;
                }
            }
            
            (mFeoDmaIdx+1 == FEO_BUFFER_NUM) ? mFeoDmaIdx = 0 : mFeoDmaIdx++;
            
            if(g_debugDump > 0)
            {
                EIS_LOG("next:mFeoDmaIdx(%u)",mFeoDmaIdx);
            }
        }
        else
        {
