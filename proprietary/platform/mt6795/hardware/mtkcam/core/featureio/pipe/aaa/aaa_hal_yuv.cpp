...
init(MINT32 i4SensorOpenIndex)
{
    ...
    m_pIHalSensor->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_EV_INFO_AWB_REF_GAIN, (MINTPTR)&ref, 0, 0);
...
onPreviewStart()
{
    ...
    {
        m_i4InitReadyCnt = 15; //getDelayFrame(EQueryType_Init);
...
MBOOL Hal3AYuv::setParams(Param_T const &rNewParam)
{
    ...
    if (m_rParam.i4MaxFps!=rNewParam.i4MaxFps||m_rParam.i4MinFps!=rNewParam.i4MinFps || m_bForceUpdatParam)
    {
        if(rNewParam.i4MinFps==rNewParam.i4MaxFps&&rNewParam.i4MaxFps>0)
        {
            ...
            setVideoFrmRate(clamp(rNewParam.i4MaxFps/1000, 5, 30));
        }
    }
...
MINT32 Hal3AYuv::updateCaptureParams(CaptureParam_T &a_rCaptureInfo)
{
    ...
    else
    {
        ...
        m_pIHalSensor->sendCommand(m_i4SensorDev, SENSOR_CMD_SET_YUV_FEATURE_CMD,(int)&yuvCmd,(int)&yuvParam,0);
    }
#endif
