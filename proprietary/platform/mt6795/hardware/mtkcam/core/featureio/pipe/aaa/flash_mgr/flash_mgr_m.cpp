...
void FlashMgrM::hw_setFlashProfile(FlashAlg* pStrobeAlg, FLASH_PROJECT_PARA* pPrjPara, NVRAM_CAMERA_STROBE_STRUCT* pNvram)
{
    ...
    int isOtg;
    int is6332;
    g_pStrobe->isChargerIn(&isCharger);
    g_pStrobe->isOtgUsing(&isOtg);
    g_pStrobe->isHw6332(&is6332);
    ...
    mCapRep.isBatLow=0;
    if((isCharger||isOtg)&&is6332)
    {
        logI("line=%d flash decL\n");
        mCapRep.isBatLow=2;
