...
MBOOL Hal3A::postCommand(ECmd_T const eCmd, MINTPTR const i4Arg)
{
	...
	bRet = sem_wait_relativeTime(&semPDThdEnd, SEM_TIME_LIMIT_NS, "postCommand::semPDThdEnd");
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
if (ENABLE_ISPTUNE & m_3ACtrlEnable) {
        IspTuningMgr::getInstance().setFlashDynamicInfo(m_i4SensorDev, FlashMgr::getInstance().isAFLampOn(m_i4SensorDev)); //prepare status for next frame
...
{
    IspTuningMgr::getInstance().setFlashDynamicInfo(m_i4SensorDev, FlashMgr::getInstance().isFlashOnCapture(m_i4SensorDev)); //prepare status for CaptureEnd
