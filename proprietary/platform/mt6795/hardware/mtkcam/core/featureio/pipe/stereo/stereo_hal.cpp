...
#define MY_LOGI(fmt, arg...)    XLOGI("[%s]" fmt, __func__, ##arg)
#define MY_LOGW(fmt, arg...)    XLOGW("[%s] WRN(%5d):" fmt, __func__, __LINE__, ##arg)
#define MY_LOGE(fmt, arg...)    XLOGE("[%s] %s ERROR(%5d):" fmt, __func__,__FILE__, __LINE__, ##arg)
...
const MUINT RefocusOffsetX      = 256;
const MUINT RefocusOffsetY      = 72;
...
#if ENABLE_ALGO
    Result = LoadFromNvram();
    MY_LOGD("SaveToNvram before");
//        for(int i=0;i<MTK_STEREO_KERNEL_NVRAM_LENGTH;i++)        
//            MY_LOGD("%d. ", pVoidGeoData->StereoNvramData.StereoData[i]);
    err = m_pStereoDrv->StereoKernelFeatureCtrl(STEREO_KERNEL_FEATURE_LOAD_NVRAM, (void*)&pVoidGeoData->StereoNvramData.StereoData, NULL);
    if (err)
    {
    ...
    //gDataArray[4] =4;
    //gDataArray[5] =5;
    MY_LOGD("StereoHal init NVRAM %d. %d. %d. %d. %d.", pVoidGeoData->StereoNvramData.StereoData[0],pVoidGeoData->StereoNvramData.StereoData[1],
	pVoidGeoData->StereoNvramData.StereoData[2],pVoidGeoData->StereoNvramData.StereoData[3],pVoidGeoData->StereoNvramData.StereoData[4]);
#endif
...
StereoHal::uninit()
{
    ...
    else if (mUsers == 1)   // Last user, must do some un-init procedure.
    {
        ...
        err = m_pStereoDrv->StereoKernelFeatureCtrl(STEREO_KERNEL_FEATURE_SAVE_NVRAM, (void*)&pVoidGeoData->StereoNvramData.StereoData, NULL);
        if (err)
        {
            MY_LOGE("StereoKernelFeatureCtrl(STEREO_KERNEL_FEATURE_SAVE_NVRAM) fail. error code: %d.", err);
            Result = MFALSE;
        }
        MY_LOGD("SaveToNvram after");
//        for(int i=0;i<MTK_STEREO_KERNEL_NVRAM_LENGTH;i++)        
//            MY_LOGD("%d. ", pVoidGeoData->StereoNvramData.StereoData[i]);
        Result = SaveToNvram();
#endif
...
//    pPipHal->GetSensorMaxFrameRate(main2_idx, capFps_Main2, prvFps_Main2);  // main2 always full size(capFps)
    ...
    OutData.stereoSize     = (char*)"4352x1152";         
...
********************************************************************************/
MSize
StereoHal::getAlgoInputSize(MUINT32 const idx) const
{
...
#else
    return (0 == idx) ? MSize(1920, 1080) : MSize(1920, 1080);
#endif
}
...
{
#if ENABLE_ALGO
    OutData.rrz_crop_main1  = MRect(MPoint(gStereoKernelInitInfo.remap_main.rrz_offset_x,gStereoKernelInitInfo.remap_main.rrz_offset_y), MSize(gStereoKernelInitInfo.remap_main.rrz_usage_width,gStereoKernelInitInfo.remap_main.rrz_usage_height));
    OutData.rrz_size_main1  = getRrzSize(0); //MSize(gStereoKernelInitInfo.remap_main.rrz_out_width,gStereoKernelInitInfo.remap_main.rrz_out_height);
    OutData.rrz_crop_main2  = MRect(MPoint(gStereoKernelInitInfo.remap_minor.rrz_offset_x,gStereoKernelInitInfo.remap_minor.rrz_offset_y), MSize(gStereoKernelInitInfo.remap_minor.rrz_usage_width,gStereoKernelInitInfo.remap_minor.rrz_usage_height));
    OutData.rrz_size_main2  = getRrzSize(1); //MSize(gStereoKernelInitInfo.remap_minor.rrz_out_width,gStereoKernelInitInfo.remap_minor.rrz_out_height);
#else
...
StereoHal::STEREOInit(INIT_DATA_STEREO_IN_T InData, INIT_DATA_STEREO_OUT_T &OutData)
{
    ...
    MBOOL Result = MTRUE;
    MINT32 err = 0; // 0: no error. other value: error.
    m_pWorkBuf = NULL; // Working Buffer
    m_pDebugInforkBuf = NULL;
    //
    ...
    ratio_x = 1.0f + DEFAULT_CROP_PRERCENT_X;
    ratio_y = 1.0f + DEFAULT_CROP_PRERCENT_Y;
    
#if ENABLE_ALGO
    main1_sensor_index = InData.main1_sensor_index;
    MUINT sensorIdx = InData.main1_sensor_index;
    IHalSensorList* sensorlsit = IHalSensorList::get();
    IHalSensor* pIHalSensor = sensorlsit->createSensor("Stereo hal", sensorIdx);	
    int sensorDevIdx = sensorlsit->querySensorDevIdx(sensorIdx);
    MY_LOGD("main1 sensorDevIdx %d. sensorIdx %d",sensorDevIdx,sensorIdx); 

    gStereoKernelInitInfo.main_source_image_width = InData.main_image_size.w * ratio_x;      
    ...
    gStereoKernelInitInfo.algo_source_image_width = 1920 * ratio_x;
    gStereoKernelInitInfo.algo_source_image_height = 1080 * ratio_y;   
    gStereoKernelInitInfo.algo_output_image_width = 1920 ; 
    gStereoKernelInitInfo.algo_output_image_height = 1080; 
    ...
    mAlgoAppendSize.w = gStereoKernelInitInfo.algo_source_image_width  + RefocusOffsetX; 
    mAlgoAppendSize.h = gStereoKernelInitInfo.algo_source_image_height + RefocusOffsetY;    
    ...
    //Get min/max dac
    mpHal3A = IHal3A::createInstance(IHal3A::E_Camera_1, sensorIdx, "MTKStereoCamera"); 
    ...
    {
        //Set algo image size
        gStereoKernelInitInfo.algo_source_image_width   = InData.algo_image_size.w * ratio_x - RefocusOffsetX;
        gStereoKernelInitInfo.algo_source_image_height  = InData.algo_image_size.h * ratio_y - RefocusOffsetY;   
        ...
        if(main2_FOV_horizontal > (main1_FOV_horizontal + 10))//FOV diff bigger than 10 will do precrop
        {
            gStereoKernelInitInfo.remap_minor.rrz_usage_width  = (MINT32)( tan((main1_FOV_horizontal+10)/180.0f*acos(0)) / tan(main2_FOV_horizontal/180.0f*acos(0)) * rSensorCropInfo.w2_tg_size/32 )*32 ;
            gStereoKernelInitInfo.hori_fov_minor = main1_FOV_horizontal + 10 ;
...
//debug ----------------------------------------------------------------------------------------------------------  
    ...
    MY_LOGD("sensor_offset_x0 %d.",gStereoKernelInitInfo.remap_main.sensor_offset_x0);
    MY_LOGD("sensor_offset_y0 %d.",gStereoKernelInitInfo.remap_main.sensor_offset_y0);
    MY_LOGD("sensor_size_w0 %d.",gStereoKernelInitInfo.remap_main.sensor_size_w0);
    MY_LOGD("sensor_size_h0 %d.",gStereoKernelInitInfo.remap_main.sensor_size_h0);
    MY_LOGD("sensor_scale_w %d.",gStereoKernelInitInfo.remap_main.sensor_scale_w);
    MY_LOGD("sensor_scale_h %d.",gStereoKernelInitInfo.remap_main.sensor_scale_h);
    MY_LOGD("sensor_offset_x1 %d.",gStereoKernelInitInfo.remap_main.sensor_offset_x1);
    MY_LOGD("sensor_offset_y1 %d.",gStereoKernelInitInfo.remap_main.sensor_offset_y1);
    MY_LOGD("tg_offset_x %d.",gStereoKernelInitInfo.remap_main.tg_offset_x);
    MY_LOGD("tg_offset_y %d.",gStereoKernelInitInfo.remap_main.tg_offset_y);
    MY_LOGD("rrz_offset_x %d.",gStereoKernelInitInfo.remap_main.rrz_offset_x);
    MY_LOGD("rrz_offset_y %d.",gStereoKernelInitInfo.remap_main.rrz_offset_y);
    MY_LOGD("rrz_out_width %d.",gStereoKernelInitInfo.remap_main.rrz_out_width);
    MY_LOGD("rrz_out_height %d.",gStereoKernelInitInfo.remap_main.rrz_out_height);
    MY_LOGD("rrz_usage_width %d.",gStereoKernelInitInfo.remap_main.rrz_usage_width);
    MY_LOGD("rrz_usage_height %d.",gStereoKernelInitInfo.remap_main.rrz_usage_height);
    MY_LOGD("pixel_array_width %d.",gStereoKernelInitInfo.remap_minor.pixel_array_width);
    MY_LOGD("pixel_array_height %d.",gStereoKernelInitInfo.remap_minor.pixel_array_height);
    MY_LOGD("sensor_offset_x0 %d.",gStereoKernelInitInfo.remap_minor.sensor_offset_x0);
    MY_LOGD("sensor_offset_y0 %d.",gStereoKernelInitInfo.remap_minor.sensor_offset_y0);
    MY_LOGD("sensor_size_w0 %d.",gStereoKernelInitInfo.remap_minor.sensor_size_w0);
    MY_LOGD("sensor_size_h0 %d.",gStereoKernelInitInfo.remap_minor.sensor_size_h0);
    MY_LOGD("sensor_scale_w %d.",gStereoKernelInitInfo.remap_minor.sensor_scale_w);
    MY_LOGD("sensor_scale_h %d.",gStereoKernelInitInfo.remap_minor.sensor_scale_h);
    MY_LOGD("sensor_offset_x1 %d.",gStereoKernelInitInfo.remap_minor.sensor_offset_x1);
    MY_LOGD("sensor_offset_y1 %d.",gStereoKernelInitInfo.remap_minor.sensor_offset_y1);
    MY_LOGD("tg_offset_x %d.",gStereoKernelInitInfo.remap_minor.tg_offset_x);
    MY_LOGD("tg_offset_y %d.",gStereoKernelInitInfo.remap_minor.tg_offset_y);
    MY_LOGD("rrz_offset_x %d.",gStereoKernelInitInfo.remap_minor.rrz_offset_x);
    MY_LOGD("rrz_offset_y %d.",gStereoKernelInitInfo.remap_minor.rrz_offset_y);
    MY_LOGD("rrz_out_width %d.",gStereoKernelInitInfo.remap_minor.rrz_out_width);
    MY_LOGD("rrz_out_height %d.",gStereoKernelInitInfo.remap_minor.rrz_out_height);
    MY_LOGD("rrz_usage_width %d.",gStereoKernelInitInfo.remap_minor.rrz_usage_width);
    MY_LOGD("rrz_usage_height %d.",gStereoKernelInitInfo.remap_minor.rrz_usage_height);
...
//debug ----------------------------------------------------------------------------------------------------------   
    ...
    stStereodepthHalInitParam.pNvRamDataArray       = pVoidGeoData->StereoNvramData.DepthAfData;
...
StereoHal::STEREOSetParams(SET_DATA_STEREO_T RunData)
{
    ...
    MY_LOGD("af_win_start_x_remap = %d",gStereoKernelProcInfo.af_win_start_y_remap);
    MY_LOGD("af_win_end_x_remap = %d",gStereoKernelProcInfo.af_win_end_y_remap);
...
StereoHal::_stereoDepthAFGetResult(MBOOL bEnableAlgo)
{
        ...
        STEREO_KERNEL_SCENARIO_IMAGE_CAPTURE_RF == gStereoKernelInitInfo.scenario)
    {
        ...
        *(temp+10)= ((m_rfsWindow.start_x + m_rfsWindow.end_x)>>1);
        *(temp+11)= ((m_rfsWindow.start_y + m_rfsWindow.end_y)>>1);
        
        MUINT8 *pDebugImageOffset = m_pDebugInforkBuf + 48;
...
lb_Abnormal_Exit:
    ...
    if(0 == m_uSensorRelativePosition)
    {
        ...
        OutData.algo_main1.s    = getAlgoImgSize(0);
        ...
        OutData.algo_main2.s    = getAlgoImgSize(1);
    }   
    else
    {
    ...
    OutData.algo_main2.s     = getAlgoImgSize(0); 
#endif
...
StereoHal::LoadFromNvram()
{
    ...
    // memcpy(arrNVRAM,pVoidGeoData->Data,MAXIMUM_NVRAM_CAMERA_GEOMETRY_FILE_SIZE); // Aaron
lb_Abnormal_Exit:
