...
onInit()
{
    ...
    //
    //
    if( muCapParamCount > 0 ) // hdr
    {
        mpIspSyncCtrl->addPass1ReplaceBuffer(muShotCount > 3 ? (muShotCount-2) : 1, 0);
    }
    else // mfll
    {
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    {    
        ...
        if( getFlag(DO_CAPTURE) )
        {
            ...
            MUINT32 eisDivH, eisDivV;
            MUINT32 eisInW, eisInH;
            MFLOAT mfll_step_v, mfll_step_h;
            eisDivH = mpEisHal->GetEisDivH();
            eisDivV = mpEisHal->GetEisDivV();
            mpEisHal->GetEisInputSize(&eisInW,&eisInH);
            MY_LOGD("EIS DIV(%d,%d)EIS win(%d,%d)", eisDivH, eisDivV, eisInW, eisInH);
            if(eisInW != 0 && eisInH != 0){
                mfll_step_h = (MFLOAT)eisDivH * pImageBuffer->getImgSize().w / eisInW;
                mfll_step_v = (MFLOAT)eisDivV * pImageBuffer->getImgSize().h / eisInH;
            }
            MY_LOGD("MFLL EIS step(%f,%f)", mfll_step_h,mfll_step_v);
            
        	for(MUINT32 j=0; j<EIS_MAX_WIN_NUM; j++)
        	{
        	    pEisStat->i4LMV_X[j] = (MFLOAT)pEisStat->i4LMV_X[j] * mfll_step_h;
        	    pEisStat->i4LMV_Y[j] = (MFLOAT)pEisStat->i4LMV_Y[j] * mfll_step_v;                
        	}               
            handlePostBuffer(CONTROL_EISDATA, (MUINTPTR)pEisStat, 0);
        }
    }
