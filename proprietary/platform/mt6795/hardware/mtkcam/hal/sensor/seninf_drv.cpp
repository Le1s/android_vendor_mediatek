...
int SeninfDrvImp::init()  //kk test
{
    ...
    mpCSI2RxAnalogRegStartAddr = (unsigned int *) mmap(0, CAM_MIPIRX_ANALOG_RANGE, (PROT_READ|PROT_WRITE), MAP_SHARED, mfd, mipiRx_analog);
    ...
    mpCAMIODrvRegAddr = mpGpioHwRegAddr + (0xB20 / 4);//GPIO210 -> CMMCLK
...
)
{
    ...
    if(ncsi2En == 1) {  // enable CSI2
      ...
      /*if(Isp_clk == 317)
        ...
        LOG_MSG("TermDelay:%d SettleDelay:%d ClkTermDelay:%d Vsync:%d lane_num:%d CSI2_en:%d HeaderOrder:%d SettleDelay(cnt):%d ISP_clk:%d\n",
            (int) dataTermDelay, (int) dataSettleDelay, (int) clkTermDelay, (int) vsyncType, (int) dlaneNum,
            (int) ncsi2En, (int)dataheaderOrder, (int)msettleDelay, (int)Isp_clk);
        ...
        else // for NCSI2
        {
            ...
            temp = SENINF_READ_REG(pSeninf, SENINF1_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF1_NCSI2_CTL,(temp|(1<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
#else
            temp = (msettleDelay&0xFF)<<8;  //Fixed@145ns/4ns(ISP clock)
            SENINF_WRITE_REG(pSeninf, SENINF1_NCSI2_LNRD_TIMING, temp);
            temp = SENINF_READ_REG(pSeninf, SENINF1_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF1_NCSI2_CTL,(temp|(dataheaderOrder<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
#endif
    ...
    //ToDo: add GPIO config & MIPI RX config
     ...
     if(ncsi2En == 1) {  // enable CSI2
        ...
        else // for NCSI2
        {
            ...
            temp = SENINF_READ_REG(pSeninf, SENINF2_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF2_NCSI2_CTL,(temp|(1<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
#else
            temp = (msettleDelay&0xFF)<<8;  //Fixed@145ns/4ns(ISP clock)
            SENINF_WRITE_REG(pSeninf, SENINF2_NCSI2_LNRD_TIMING, temp);

            temp = SENINF_READ_REG(pSeninf, SENINF2_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF2_NCSI2_CTL,(temp|(dataheaderOrder<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
#endif
    ...
    //ToDo: add GPIO config & MIPI RX config
     ...
     if(ncsi2En == 1) {  // enable CSI2
        ...
        else // for NCSI2
        {
            ...
            temp = SENINF_READ_REG(pSeninf, SENINF2_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF3_NCSI2_CTL,(temp|(1<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
            ...
            temp = SENINF_READ_REG(pSeninf, SENINF3_NCSI2_CTL);
            SENINF_WRITE_REG(pSeninf, SENINF3_NCSI2_CTL,(temp|(dataheaderOrder<<16)|(ncsi2En<<4)|(((1<<(dlaneNum+1))-1)))) ;
#endif
...
int SeninfDrvImp::setMclk1IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        *(mpCAMIODrvRegAddr) &= 0xFFFFF9FF;
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<9);
    }
...
int SeninfDrvImp::setMclk2IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        *(mpCAMIODrvRegAddr) &= 0xFFFFFFF9;
       *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<1);
    }
//    LOG_MSG("[setIODrivingCurrent]:%d 0x%08x\n", (int) ioDrivingCurrent, (int) (*(mpCAMIODrvRegAddr)));
