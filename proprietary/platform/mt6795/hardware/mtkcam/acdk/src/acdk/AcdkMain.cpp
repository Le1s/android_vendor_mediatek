...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ...
        m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH, &mPrvIMemInfo[i]);
        m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH, &mDispIMemInfo[i]);
    }
    ...
    for(MINT32 i = 0; i < SURFACE_NUM; ++i)
    {
        ...
        m_pIMemDrv->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_FLUSH, &mSurfaceIMemInfo[i]);
    }
                ...
                MINT32  const isSaveImg)
{
    ...
    //ACDK_LOGD("mCapIMemInfo.virtAddr(0x%lx), PureRaw_BufAddr(0x%p), mCapIMemInfo.size(0x%d)", mCapIMemInfo.virtAddr,PureRaw_BufAddr,mCapIMemInfo.size);
    //memcpy((MVOID*)mCapIMemInfo.virtAddr,PureRaw_BufAddr,mCapIMemInfo.size);
...
void AcdkMain::cameraCallback(void* param)
{
    ...
    acdkCbParam.width    = pCBInfo->mData1;
    acdkCbParam.height   = pCBInfo->mData2;
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"PreCB width(%d),heidht(%d)",pCBParam->width,pCBParam->height);
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(%p),PA(%p),Size(0x%x)",mSurfaceIndex,mSurfaceIMemInfo[mSurfaceIndex].virtAddr,mSurfaceIMemInfo[mSurfaceIndex].phyAddr,mSurfaceIMemInfo[mSurfaceIndex].size);
    ...
    else
    {
        if(mOrientation == 90 || mOrientation == 270)
        {
            height = pCBParam->width;
            width = pCBParam->height;
        }
        else
        {
            width = pCBParam->width;
            height = pCBParam->height;
        }
    }
    ...
    if(dumpEnable == 2 || dumpEnable == 3)
    {
        ...
        ACDK_LOGD("Save image file name:%s, w(%d), h(%d)", szFileName, width, height);
    ...
    else
    {
        ...
        MUINT32 displaySize = width*height*3/2;
...
)
{
    ...
    else if(a_u4Ioctl == ACDK_CMD_PREVIEW_START)
    {
        ...
        ACDK_LOGD("Preview OP Mode:%d", pCamPreview->eOperaMode);

        mHDR_EN = pCamPreview->u16HDRModeEn;

        mTestPatternOut = pCamPreview->u16PreviewTestPatEn;
        mOperaMode = pCamPreview->eOperaMode;
