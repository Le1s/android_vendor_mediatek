...
takePicture()
{
    ...
    //
    {
        ...
        {
            // (1) disable preview. return Pass1 buffer immediately.
            if(mpSyncNode)
            {
...
#define parse_pair(string, first, second, delimeter)                        \
        do {                                                                \
            ...
            *second = h;                                                    \
            ...
            String8 s8RefocusSize   = pParamsMgr->getStr(MtkCameraParameters::KEY_REFOCUS_PICTURE_SIZE);
            String8 s8StereoSize    = pParamsMgr->getStr(MtkCameraParameters::KEY_STEREO_PICTURE_SIZE);
            if  ( String8::empty() == s8RefocusSize ) {
                s8RefocusSize = String8("3072x1728");
                MY_LOGW("Refocus size is empty, change to %s", s8RefocusSize.string());
            }
            if  ( String8::empty() == s8StereoSize ) {
                s8StereoSize = String8("4352x1144");
                MY_LOGW("Stereo size is empty, change to %s", s8StereoSize.string());
            }
            //
            int iPictureWidth = 0, iPictureHeight = 0;
            parse_pair(s8RefocusSize.string(), &iPictureWidth, &iPictureHeight, 'x');
            //
