...
{ 
    ...
    MBOOL isPostPreviewEnabled = isDataMsgEnabled(NSShot::EIShot_DATA_MSG_POSTVIEW);
    MY_LOGD("@@@ isPostPreviewEnabled= %d",isPostPreviewEnabled);
    if (isPostPreviewEnabled)
    {
...
fgCamShotDataCb(MVOID* user, NSCamShot::CamShotDataInfo const msg)
{
    ...
    if (NULL != pCapBufShot) 
    {
        ...
        {
            pCapBufShot->handlePostViewCB(msg.pBuffer);
        }
...
MBOOL
CapBufShot::           
handlePostViewCB(IImageBuffer const *pImgBuffer)
{
    AutoCPTLog cptlog(Event_Shot_handlePVData);
    MY_LOGD("+ (pImgBuffer) = (%p)", pImgBuffer); 
    mpShotCallback->onCB_Postview(reinterpret_cast<IImageBuffer const*>(pImgBuffer)); 
    return  MTRUE;
}
