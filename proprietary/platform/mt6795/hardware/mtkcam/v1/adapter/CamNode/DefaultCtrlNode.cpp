...
//
// for ZSD extension
#include <mtkcam/v1/adapter/inc/Scenario/Shot/ScenarioShotUtils.h>
...
onInit()
{
    ...
    if(mpIspSyncCtrl)
    {
        ...
        if (mMode != MODE_STEREO_PREVIEW || mMode != MODE_STEREO_ZSD_PREVIEW || mMode != MODE_STEREO_VDO_PREVIEW)
        {
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    switch(data)
    {
        ...
        {
            //MY_LOGD("on post preview buf = 0x%x", buf);
...
setParameters()
{
    ...
    if(mspParamsMgr != NULL)
    {
        ...
        {
            char value[PROPERTY_VALUE_MAX] = {'\0'}; 
            property_get( "debug.forceRecBitStream.enable", value, "-1");
            MINT32 val = atoi(value);
            if( val > -1 )
            {
                forceRecBitStream = val;
                MY_LOGD("forceRecBitStream(%d)", forceRecBitStream);
            }
        }
        //
        if( forceRecBitStream )
        {
