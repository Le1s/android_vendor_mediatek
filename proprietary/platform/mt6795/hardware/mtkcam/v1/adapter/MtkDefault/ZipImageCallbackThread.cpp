...
threadLoop()
{
    ...
    switch(cbtype)
    {
        ...
        case callback_type_post_view_yuv:
            {
                callback_data one;
                //
                if( !getCallbackData(callback_type_post_view_yuv, &one) )
                {
                    break;
                }
                //
                return handlePostViewYUVCallback(one);
            }
...
bool
ZipImageCallbackThread::
handlePostViewYUVCallback(callback_data const& data)
{
