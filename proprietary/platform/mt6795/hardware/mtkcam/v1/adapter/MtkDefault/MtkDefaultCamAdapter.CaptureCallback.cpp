 ...
 ******************************************************************************/
//
//  Callback of Postview data (CAMERA_MSG_POSTVIEW_FRAME)
//
//  Arguments:
//
//      puPostviewBuf
...
bool
CamAdapter::
onCB_Postview(NSCam::IImageBuffer const *pPostviewBuf)
{
    MY_LOGD("+");
    if  ( msgTypeEnabled(CAMERA_MSG_POSTVIEW_FRAME) )
    {
        using namespace NSCam::NSIoPipe::NSSImager; 
        MY_LOGD("buf=%p", pPostviewBuf);
        
        IImageBuffer const *scrBuffer = pPostviewBuf;
        
        // for preview call back
        {            
            MY_LOGD("scrFormat = %d", scrBuffer->getImgFormat());
            MY_LOGD("scrSize w = %d, h = %d", scrBuffer->getImgSize().w, scrBuffer->getImgSize().h);
            MY_LOGD("scrPlaneCount = %d", scrBuffer->getPlaneCount());
            int iPreviewWidth = 0, iPreviewHeight = 0;

            // prepare an IImageBuffer, and use SImager to copy buffer from source
            sp<IParamsManager> pParamsMgr = getParamsManager();        
            pParamsMgr->getPreviewSize(&iPreviewWidth, &iPreviewHeight);
                
            String8 const sPreviewFormat = mpParamsMgr->getPreviewFormat();

            IImageBufferAllocator* allocator = IImageBufferAllocator::getInstance();
            sp<IImageBuffer> DesBuf = NULL;

            MINT32 bufBoundaryInBytes_1[] = {0,0,0};
            MUINT32 planeCount = 0, bufStridesInBytes_1[] = {0,0,0};
            MINT32 fmt = Format::queryImageFormat(sPreviewFormat);
            MY_LOGD("des fmt = %d", fmt);
            planeCount = Format::queryPlaneCount(fmt);
            MY_LOGD("des planeCount = %d", planeCount);
            for (MUINT32 i = 0; i < planeCount; i++)
            {
                bufStridesInBytes_1[i] = 
                    (Format::queryPlaneWidthInPixels(fmt,i, iPreviewWidth)*Format::queryPlaneBitsPerPixel(fmt,i))>>3;
                //MY_LOGD("des bufStridesInBytes_1[%d] = %d",i, bufStridesInBytes_1[i]);
            }

            IImageBufferAllocator::ImgParam imgParam(
                        fmt, 
                        MSize(iPreviewWidth,iPreviewHeight), 
                        bufStridesInBytes_1,
                        bufBoundaryInBytes_1,
                        planeCount);

            DesBuf = allocator->alloc_ion(LOG_TAG, imgParam);

            DesBuf->lockBuf(LOG_TAG, eBUFFER_USAGE_HW_MASK|eBUFFER_USAGE_SW_MASK);;
            
            static ISImager *g_pISImager = NULL;
            g_pISImager = ISImager::createInstance(scrBuffer);

            g_pISImager->setTargetImgBuffer(&(*DesBuf)); 

            g_pISImager->execute();
            #if 0
            char filename[256];      
            sprintf(filename, "/sdcard/resultTest.yuv");      
            DesBuf->saveToFile(filename);
            #endif
            DesBuf->unlockBuf(LOG_TAG); 
            

            DesBuf->lockBuf(LOG_TAG, eBUFFER_USAGE_SW_MASK);        
            
            MUINT u4Size = 0;
            for (int i=0; i<planeCount; i++)
            {
                u4Size += DesBuf->getBufSizeInBytes(i);
            }
            
            // copy to call back data
            camera_memory* pmem = NULL;
            pmem = mpCamMsgCbInfo->mRequestMemory(-1, u4Size, 1, NULL);
            //MY_LOGD("u4Size = %d", u4Size);

            if (pmem)
            {
                MUINT8* puTmp = (MUINT8*)pmem->data;
                memcpy(puTmp, (MUINT8 *)DesBuf->getBufVA(0), DesBuf->getBufSizeInBytes(0));
                for (int i = 1; i < DesBuf->getPlaneCount(); i++)
                {
                    puTmp = puTmp + DesBuf->getBufSizeInBytes(i - 1);
                    memcpy(puTmp, (MUINT8 *)DesBuf->getBufVA(i), DesBuf->getBufSizeInBytes(i));
                }
                //MY_LOGD("Copy to mem");
            }

            DesBuf->unlockBuf(LOG_TAG);
            allocator->free(DesBuf.get());
            DesBuf = NULL;
            ...
            if  ( pmem )
            {
                ...
                {
                    ZipImageCallbackThread::callback_type_post_view_yuv,
                    pmem,
                    0,
                    false
                };
                ...
                //
                pThread->doCallback(ZipImageCallbackThread::callback_type_post_view_yuv);
                pThread = NULL;
            }
        }
        MY_LOGD("-");
