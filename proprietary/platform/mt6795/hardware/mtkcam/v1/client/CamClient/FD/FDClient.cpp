...
*/
    ...
    startHeartrateDetection(::android_atomic_release_load(&mIsHRPreStarted));
    ...
    case CAMERA_CMD_SETCB_HR_PREVIEW:
        {
            ...
            MY_LOGD("WillTest Set HR Callback : %X", ptr);
            MY_LOGD("WillTest Set HR Callback 1: %p", (HeartrateCallback_t)ptr);
    ...
    case CAMERA_CMD_SETUSER_HR_PREVIEW:
        {
            MUINTPTR ptr;
            ptr = (MUINTPTR)(((MUINTPTR)((MUINT32)arg1)) + (((MUINTPTR)((MUINT32)arg2)) << 32));
            MY_LOGD("WillTest Set HR user : %X", ptr);
...
bool
FDClient::
performHRCallback(void *HR_result)
{
    bool ret = true;

    if(mHeartrateCb && mpHeartrateUser && HR_result) {
