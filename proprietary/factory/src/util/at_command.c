...
int open_SDIO()
{
    ...
    {
         LOGD(TAG "Support C2K modem");
         fd_c2k = openDeviceWithDeviceName("/dev/ttySDIO3");
         if (-1 == fd_c2k)
         {
	       		LOGD(TAG "Fail to open SDIO interface\n");
	       		return -1;
         }
         LOGD(TAG "Success to open SDIO interface\n");
...
int C2Kmodemsignaltest(int fd)
{
    LOGD(TAG "[AT]Detect C2KMD active status:\n");
    send_at (fd, "ate0q0v1\r\n");
    do
    {
    	send_at (fd, "AT\r\n");
    ...
    LOGD(TAG "[AT]Check the SIM status:\n");
		...
		if (send_at (fd, "AT+CREG=1\r\n")) goto err;
		if(wait4_ack (fd, "+CREG", 30000)) goto err;
...
const char* dial112(const int fd)
{
	...
	if(wait4_ack2(fd, "+ECPI: 1,6", "ESPEECH", "+ECPI: 1,2", 30000))
	{
...
int dial112C2K(const int fd)
{
    ...
    LOGD(TAG "[AT]Dail Up 112:\n");
	  send_at (fd, "AT+CHV\r\n");
	  wait4_ack (fd, NULL, 3000);
	  LOGD(TAG ">>>>>>>>> AT+CHV done:\n");
	  
	  LOGD(TAG "[AT]Detect SIM status:\n");
	  if (send_at (fd, "AT+ESIMS\r\n")) goto err;
		if (wait4_ack (fd, NULL, 30000)) goto err;
	  
	  if (send_at (fd, "AT+CDV=112\r\n"))
	  {
			LOGD(TAG ">>>>>>>>> send ATD112 error\n");
			goto err;
	  }
	  LOGD(TAG ">>>>>>>>> send AT+CDV=112 done:\n");
    ...
    if(wait4_ack (fd, "^CONN", 15000))
		{
			LOGD(TAG ">>>>>>>>> wait for ^CONN error\n");
			goto err;
	  }
	  LOGD(TAG ">>>>>>>>> wait for ^CONN done:\n");	
