...
/**
 * phone RAT family , mapping to PHONE_RAT_FAMILY_NONE, PHONE_RAT_FAMILY_2G, PHONE_RAT_FAMILY_3G,
 * PHONE_RAT_FAMILY_4G in PhoneRatFamily
 */
typedef enum {
    PHONE_RAT_FAMILY_NONE = 0x00,
    PHONE_RAT_FAMILY_2G = 0x01,
    PHONE_RAT_FAMILY_3G = 0x02,
    PHONE_RAT_FAMILY_4G = 0x04,
} RIL_PhoneRatFamily;
...
/**
 * RIL_REQUEST_GET_PHONE_RAT_FAMILY
 *
 * Used to get phone RAT family
 *
 * "data" is int *
 * ((int *)data)[0] is the set of RIL_PhoneRatFamily bits currently enabled.
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_GET_PHONE_RAT_FAMILY 130

/**
 * RIL_REQUEST_SET_PHONE_RAT_FAMILY
 *
 * Used to set the phones RAT Families. Be VERY careful using this request
 * as it may cause some vendor modems to reset. Because of the possible modem
 * reset any RIL commands after this one may not be processed.
 *
 * "data" is one or more RIL_PhoneRatFamily bits that the modem is allowed to use.
 * "response" is NULL
 *
 * Valid errors:
 *  SUCCESS means a RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE will be sent within 30 seconds.
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_SET_PHONE_RAT_FAMILY 131
...
/**
 * RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE
 *
 * Sent when RIL_REQUEST_SET_PHONE_RAT_FAMILY completes. Returns the set of
 * RIL_PhoneRatFamily bits exactly as RIL_REQUEST_GET_PHONE_RAT_FAMILY and
 * should be the same set as sent by RIL_REQUEST_SET_PHONE_RAT_FAMILY. If no
 * bits set, this indicates an error.
 *
 * "data" is int *
 * ((int *)data)[0] is the set of RIL_PhoneRatFamily bits currently enabled.
 *
 */
#define RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE 1042
