...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t, RIL_SOCKET_ID socket_id)
{
    ...
    if (s_md_off &&
            ...
            request != RIL_REQUEST_GET_PHONE_RAT_FAMILY &&
    ...
    if (radioState == RADIO_STATE_UNAVAILABLE &&
            ...
            request != RIL_REQUEST_GET_PHONE_RAT_FAMILY &&
            request != RIL_REQUEST_SET_PHONE_RAT_FAMILY &&
...
void sendSetPhoneRatUrc(int sim3G)
{
    ...
    switch(sim3G) {
        ...
        case CAPABILITY_3G_SIM2:
            RIL_onUnsolicitedResponseSocket(RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE, urcData, sizeof(int)*2, RIL_SOCKET_1);
    #if (SIM_COUNT >= 3)
            RIL_onUnsolicitedResponseSocket(RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE, urcData, sizeof(int)*2, RIL_SOCKET_3);
    #endif
    #if (SIM_COUNT >= 4)
            RIL_onUnsolicitedResponseSocket(RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE, urcData, sizeof(int)*2, RIL_SOCKET_4);
    #endif
            if(isLteSupport()) {
                urcData[0] = PHONE_RAT_FAMILY_2G | PHONE_RAT_FAMILY_3G | PHONE_RAT_FAMILY_4G;
            } else {
                urcData[0] = PHONE_RAT_FAMILY_2G | PHONE_RAT_FAMILY_3G;
            }
            RIL_onUnsolicitedResponseSocket(RIL_UNSOL_SET_PHONE_RAT_FAMILY_COMPLETE, urcData, sizeof(int)*2, RIL_SOCKET_2);
        break;
...
static void *mainLoop(void *param)
{
    ...
    while (s_main_loop) {
        ...
        //int oldSim3G = RIL_get3GSIM();
        //if(sim3G != oldSim3G)
        {
            sendSetPhoneRatUrc(sim3G);
        }
