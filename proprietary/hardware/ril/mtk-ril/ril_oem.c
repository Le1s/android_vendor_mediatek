...
extern void requestGetPhoneRatFamily(void * data, size_t datalen, RIL_Token t)
{
    int response = PHONE_RAT_FAMILY_2G;

#ifndef MTK_GEMINI
    /* Only Gemini modem support AT+ES3G */
    RIL_onRequestComplete(t, RIL_E_SUCCESS, &response, sizeof(int));
#else
    response = RIL_get3GSIM();
    RIL_SOCKET_ID rid = getRILIdByChannelCtx(getRILChannelCtxFromToken(t));

    RLOGI("3G Sim : %d, current RID : %d", response, rid);

    if((int)rid == (response-1)) {
        if(isLteSupport()) {
            response = PHONE_RAT_FAMILY_2G | PHONE_RAT_FAMILY_3G | PHONE_RAT_FAMILY_4G;
        } else {
            response = PHONE_RAT_FAMILY_2G | PHONE_RAT_FAMILY_3G;
        }
    } else {
        response = PHONE_RAT_FAMILY_2G;
    }
    RIL_onRequestComplete(t, RIL_E_SUCCESS, &response, sizeof(int));
#endif // MTK_GEMINI
}
...
extern void requestSetPhoneRatFamily(void * data, size_t datalen, RIL_Token t)
{
    char * cmd;
    ATResponse *p_response = NULL;
    int err;
    ...
    RLOGI("requestSetPhoneRatFamily : %d, %d, %d", setting, old3GSim, rid);

    if (setting == PHONE_RAT_FAMILY_2G) {
        // only send ES3G in 2G->3G rid
        RLOGI("no need to do sim switch");
        RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
        return;
    }
    if (old3GSim == (int)(rid+1)) {
        RLOGI("3G capability no changed, send URC to complete");
        RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
        sendSetPhoneRatUrc(old3GSim);
        return;
    }
...
extern int rilOemMain(int request, void *data, size_t datalen, RIL_Token t)
{
    switch (request)
    {
        ...
        case RIL_REQUEST_RESET_RADIO:
            ...
            break;
        case RIL_REQUEST_GET_PHONE_RAT_FAMILY:
            requestGetPhoneRatFamily(data, datalen, t);
            break;
        case RIL_REQUEST_SET_PHONE_RAT_FAMILY:
            requestSetPhoneRatFamily(data, datalen, t);
            break;
