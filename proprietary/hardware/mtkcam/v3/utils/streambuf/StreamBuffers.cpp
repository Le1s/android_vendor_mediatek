...
StreamBufferImp(
    ...
    : UsersManager(pStreamInfo->getStreamId(), pStreamInfo->getStreamName())
...
)
{
    ...
    //
    MUINT32 const result = UsersManager::markUserStatus(userId, statusMask);
    //
    ...
    if  (
          ! isUsed
        &&  isReleased
        &&  Category::PRODUCER == getUserCategory(userId)
        )
    {
