...
//                            |(GRALLOC_USAGE_HW_TEXTURE|GRALLOC_USAGE_HW_RENDER)
    ...
    MY_LOGD_IF(
        ...
        "usageForConsumer:%#x(%s) usageForAllocator:%#x(%s) "
    ...
    //  vInputMetaBuffers <- camera3_capture_request_t::settings
    {
        ...
        if  ( request->settings ) {
            ...
            // to debug
            {
                ...
                ::property_get("debug.camera.dumpmeta.loglevel", cLogLevel, "0");
                if(strcmp(cLogLevel, "full") == 0){
                    mpMetadataConverter->dumpAll(mLatestSettings);
                } else if(strcmp(cLogLevel, "filter") == 0){
                    mpMetadataConverter->dump(mLatestSettings);
                }
            }
        }
        else {
             ...
             */
            isRepeating = MTRUE;
            MY_LOGD_IF(1, "frameNo:%u NULL settings -> most-recently submitted capture request", request->frame_number);
        }
        //
...
performCallback(CallbackParcel const& cbParcel)
{
    ...
    for (size_t i = 0; i < cbParcel.vOutputMetaItem.size(); i++) {
        ...
        IMetadata* pMetadata = rCbMetaItem.buffer->tryReadLock(LOG_TAG);
        {
            ...
            //
#endif
            ret = mpMetadataConverter->convert(*pMetadata, metadata);
            // to debug
            {
                char cLogLevel[PROPERTY_VALUE_MAX] = {'\0'};
                ::property_get("debug.camera.dumpmeta.loglevel", cLogLevel, "0");
            ...
            MY_LOGF_IF(!ret || !metadata, "fail to convert metadata:%p ret:%d", metadata, ret);
        }
