 ...
 *
 ******************************************************************************/
sp<AppStreamManager::AppMetaStreamInfo>
AppStreamManager::FrameHandler::
...
)
{
    ...
    //
    {
        ...
        if  ( pItem->pItemSet->size() == pItem->pItemSet->numReturnedStreams ) {
...
{
    if  ( 1 == pItem->pItemSet->size() - pItem->pItemSet->numReturnedStreams ) {
        ...
        while (1)
        {
            ...
            //
            if  ( 0 != pFrame->vOutputMetaItem.size() - pFrame->vOutputMetaItem.numReturnedStreams )
            {
                MY_LOGW(
                    "Block to return the final meta of frameNo:%u since frameNo:%u (%zu|%zu)",
...
)   const
{
    ...
    if  ( pFrame->errors.hasBit(HistoryBit::ERROR_SENT_FRAME) ) {
        ...
        if  ( pFrame->vOutputMetaItem.size() != pFrame->vOutputMetaItem.numReturnedStreams ) {
            return MFALSE;
        }
        //
        //No shutter timestamp has been sent.
        //No meta error has been sent.
        if  ( 0 == pFrame->timestampShutter && ! pFrame->errors.hasBit(HistoryBit::ERROR_SENT_META) ) {
...
update(ResultQueueT const& rvResult)
{
    ...
    {
        MUINT32 const frameNo = rvResult[iResult];
        ...
        if  ( itFrame == mFrameQueue.end() ) {
            MY_LOGW(
                "frameNo:%u is not in FrameQueue; its first frameNo:%u", frameNo, (*mFrameQueue.begin())->frameNo);
            itFrame = mFrameQueue.begin();
        }
    }
    //
    MUINT32 const latestResultFrameNo = rvResult[rvResult.size() - 1];
...
update(List<CallbackParcel>& rCbList)
{
    ...
    while ( itFrame != mFrameQueue.end() )
    {
        ...
        if  ( checkRequestError(**itFrame) > 0 ) {
            ...
            MY_LOGV("frameNo:%u Request Error", (*itFrame)->frameNo);
